...
| `diag_rowprobe.py` | 某個**模板列**在多家公司的命中情況，同時列出 dataframe 裡所有長得像的候選。判斷一列的 concept 對照要不要改，看這支的輸出最快 | 啟用 |
| `diag_hintsweep.py` | 掃出「`label_hint` 太窄把正確答案濾掉」的模板列——比較有 hint 與沒 hint 的命中差異。**2026-08-23 靠這支發現 hint 是最大的系統性成因**（`Cash Taxes Paid` 少 14 家、`Deferred Revenue, current` 少 12 家）。⚠ 有些 hint 是必要的（擋現金流量表末尾的租賃補充揭露列），不要看到被殺就拿掉 | 啟用 |
