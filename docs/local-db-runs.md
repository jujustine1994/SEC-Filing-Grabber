...
## ✅ 2026-09-19 重建完成：214 家 / 14,405 份 / 1.60 GB

昨天（09-18）發現 J5 那批資料全部遺失後，當晚重跑。**一次跑完、零失敗**。

| | 結果 |
|---|---|
| 公司 | **214 / 214**，全部 `reached_bottom` |
| filing | **14,405 份**、**1.60 GB** |
| pass1 | 27/27 段，更新 211、跳過 3、**失敗 0** |
| pass2 | 27/27 段，更新 11、跳過 203、**失敗 0** |
| D11 抓取缺漏 | **0 家** |
| 格式驗證 | **14,405 / 14,405（100%）** 過四道閘、0 家有問題 |
| meta 需重算 | 0 家 |
| 耗時 | 17:43 → 隔日凌晨，約 9 小時 |

**跟舊的那批（201 家 / 13,921 份 / 0.98 GB）比**：多 13 家；容量從 0.98 → 1.60 GB
是因為 schema 2 起**每份存六張表**（加了股東權益變動表、綜合損益表、封面頁），
不是資料變髒。

**pass2 更新了 11 家**——這正是「跑一輪不保證抓齊」的實例，第二輪不是形式。

工具：雙擊 `過夜更新資料庫.bat`（`scripts/overnight_update_db.ps1`），
全程不需要 AI。⚠ 當晚踩到兩個 PowerShell 5.1 的坑（中文亂碼、log 鎖檔），
**都不影響資料**，已修，見 CHANGELOG 2026-09-18（續十五）（續十六）。

---

## ⚠ 2026-09-18：J5 那批資料曾經全部遺失（歷史紀錄）

2026-09-18 發現 `%APPDATA%\SEC Financial Tools\` 整個不見了（連 `config.json`
一起）。確認過：沒有第二台機器、資源回收筒沒有、只有一個 Windows 使用者帳號。
最可能是 GUI 的「全部清除」按鈕被按過（有二次確認但沒有備份）。

**後續處理**（見 `docs/TODO.md` J8）：

- 資料庫已搬到專案資料夾 `local_db/filing_cache/`，不再放 `%APPDATA%`
  ——那個位置在 Windows 語意上是「系統可以清掉的快取」，但這份資料是花
  10 小時 SEC 網路請求換來的
- 更新名單擴到 **218 家**（原 201 家 universe ＋ 新增 40 家半導體／AI 相關
  ＋ 既有重疊），待重抓
- ⚠ 名單裡的 **ASML／STM／IFX 是 20-F 外國申報人**（只有年報沒有 10-Q），
  這工具抓的是 10-K/10-Q，**可能整家抓不到**，還沒決定要不要排除

下面 2026-09-04~09-06 那兩批的紀錄**仍然有效**，重抓時照同樣的方法跑：
分段、每批用 `audit_local_db.py` 收尾、跑完再跑一輪確認全部跳過。
