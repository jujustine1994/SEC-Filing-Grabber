...
| **`fiscal_year_end` 會不會隨時間變（最大風險）** | **不要為它另外跑一輪 50 家掃描**。改成在規則裡加一道 sanity check：選中的名目季末與發布日相距若不在 `0~70` 天內，就判定這條規則對這份 filing 不成立，**退回舊算法**。實測最大發布延遲 58 天，70 天留了緩衝。公司改過財年、或延遲申報，都會被這道檢查接住 |
