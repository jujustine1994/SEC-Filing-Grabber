...
> 已於 2026-08-20 ~ 2026-08-25 分批完成並合併 master，內容看 `docs/CHANGELOG.md`
> 搜尋「跨公司」與 G2／G7／G6 三條。**這一段只剩下面這條還沒做。**

F7. **比較的期間區間要能選到月／日，不只年**（2026-09-02 CTH 提出，
   **設計已完成、CTH 說先做另外兩件，這件排後面**）
   - 現況：「起始年／結束年」兩格只吃 4 位數年份，`comparison._filter_by_year()`
     也是只比年份
   - 設計（已跟 CTH 過）：
     - 兩格改成接受 `YYYY`／`YYYY-MM`／`YYYY-MM-DD` 三種寫法（純文字，tkinter 沒有
       原生日期選擇器，而且同一個視窗的「快照時間點」本來就是要打 YYYYMMDD）
     - 語意：篩的是**期末日**落在區間內。`2024` = 2024-01-01~2024-12-31、
       `2024-06` = 該月整月
     - `_filter_by_year()` → `_filter_by_period_end()`，比對 ISO 字串即可
       （那裡本來就有每一欄的期末日，精確篩選幾乎是免費的）
     - ⚠ **抓取窗要放寬**：`fetch_gaap_statements` 是用**申報日年份**篩的（比期末日
       粗），12 月底那季的年報隔年 2 月才申報。抓取窗前後各多留一年，精確篩交給
       欄位那層
     - 舊 config 存的是純年份，`2024` 本來就是合法輸入，不會破壞既有設定
     - i18n 四個 locale 的標籤要跟著改（起始年 → 起始）
   - 驗收要跑一次真實比較（會打 SEC），確認邊界那幾期有進有出

F6. **圖表的 X 軸改成真正的日期軸**（2026-09-02 CTH 提出，**已查清現況，未動手**）
   - **現況**：X 軸是**文字類別軸**（`<c:catAx>` + `strRef`），標籤是 `20200126` 這種
     8 碼字串、**等距**排列。所以 2020Q1→2020Q2 跟 COST 那種 16 週的季在軸上一樣寬，
     時間軸是失真的
   - **為什麼會這樣（不是 bug，是連鎖取捨）**：
     1. 「期末結算日」那一列**刻意存成文字** `"YYYYMMDD"`（`comparison_writer.py:463`），
        因為 Snapshot 的 `SUMPRODUCT`／`MATCH` 要跟使用者輸入格式對得起來
     2. openpyxl 的 `set_categories()` **不管儲存格內容是什麼，永遠寫成 `numRef`**；
        指向文字儲存格時 Excel 解析不出來，類別軸整個讀不到值、連帶圖例被擠壓變形
        （2026-08-22 CTH 截圖回報的「中間斷線＋圖例被吃」）
     3. 所以那次修復把每個 series 的 `cat` 手動換成 `StrRef` —— 圖正常了，**代價就是
        軸型別從此固定是文字類別軸**
   - **要改成日期軸，三件事要同時成立**：
     ① 類別來源是 Excel **真日期序列值**（numRef + 日期 numFmt），不是文字；
     ② 圖表 XML 用 `<c:dateAx>`（openpyxl 有 `openpyxl.chart.axis.DateAxis`），
     並設 `baseTimeUnit`／`majorUnit`；③ 若動到期末結算日那一列，**Snapshot 的公式與
     「使用者打 YYYYMMDD」的介面要一起改**
   - **修法選項**：
     - **A. 期末日列直接改成真日期**（顯示格式仍是 `yyyymmdd`），Snapshot 公式改成比
       日期序列值。一次到位，但動到 Snapshot ——那條線 2026-08-21 才為了型別對不上修過一輪
     - **B（建議）. 另外加一列「日期序列值」專供圖表用**，期末日那列一格都不動。
       風險低，代價是多一列（可以隱藏或放在資料區外）
     - C. 維持現狀（文字類別軸）
   - **⚠ 一個語意限制，改之前要想清楚**：跨公司比較的欄是**對齊過的日曆季**，同一欄
     各家的實際期末日不一樣（NVDA 7/27、AMD 6/28），那一格放的是**所有公司裡最晚**的
     那個日期。換成日期軸等於把這個近似值當成精確時間座標——單一公司沒問題，跨公司圖
     要在 `Compare_Notes` 講清楚
   - 驗收要看的：軸上相鄰兩點的間距要反映真實天數（COST 的 16 週 Q4、G6 補出來的空白欄
     最容易看出來），且圖例／軸標題不能又被擠壓（那是 2026-08-22 那輪的回歸點）
