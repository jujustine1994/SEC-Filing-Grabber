   ...
   **剩下兩個，都是這次修 G13 時挖出來的獨立問題**：
   ...
   - (a) **有些 filing 的 IS 根本沒有「當期」欄**（2026-09-20 修掉挑欄邏輯後
     ...
     **ISRG／GD 的 CF 有當期而 IS 沒有**——所以「IS 缺當期時從 CF 借期間欄」
     ...
     ⚠ 動手前先查證那幾份的 SEC 原始 XBRL 到底有沒有當期 fact——
     **是 edgartools 沒解出來，還是公司根本沒報**，兩者的修法完全不同。
   - (b) **`fiscal_quarter_of()` 對 52/53 週財年制算錯季**。它用月份切
     （`(月 - 財年起始月) % 12 // 3 + 1`），但 KR 的 Q1 是 **16 週**、
     5 月下旬才結束，月份法算成 Q2。⚠ 這個錯**會撞掉資料**：算出的季若已被
     別份 filing 佔用，`if label in periods: continue` 會整份跳過。2026-09-20
     已經加了「撞欄就退回借來的標籤、再退回裸日期」的保護（資料留著，
     標籤退化），但**季還是算錯的**。要真的修，得從 filing 的實際期末日序列
     推週制，不能只看月份。
...
J11. **財年結束月有兩個來源，實測至少一家對不上（目前還沒爆，先記著）**
   （2026-09-20 掃描時發現。**不是 bug，是潛在風險**——現在兩條路算出來的
   標籤一樣，但依據不同，哪天其中一邊變了就會靜默分岔）
   - **兩個來源**：
     - 主路徑 `_fetch_gaap_impl()` → `_detect_fy_end_month(filings_k)`，
       解 10-K／20-F 全文
     - `preview_sheets()` → `Company.fiscal_year_end` 屬性（一次請求就有，
       預覽用不起解全文那條）
   - **實測 10 家**（AAPL／NVDA／ARLO／WMT／TGT／SNOW／ARM／AVGO／ADBE／COST）：
     只有 **TGT 對不上**——屬性寫 `0201`（2 月），解析出來是 **1 月**。
     TGT 是 52/53 週制，財年結束日在 1 月底到 2 月初之間浮動
   - **為什麼現在沒事**：實測 TGT 的預覽與實抓對同一個期末日 `2026-08-01`
     都給 `FY2027Q2`——`fiscal_quarter_of()` 的邊界算法讓差一個月的兩個
     `fy_end_month` 落到同一季。**這是巧合，不是保證**
   - **要修的話**：讓預覽跟主路徑用同一個依據。但預覽的成本上限是
     「一次 HTTP 請求」，解全文那條用不起——所以不是改一行的事，要先想清楚
   - **關聯**：G13 修正時記下的「`fiscal_quarter_of()` 對 52/53 週財年制
     算錯季」是同一個家族的問題，TGT 正好也是 52/53 週制
