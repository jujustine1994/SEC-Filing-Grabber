...
G13. **期間欄挑錯／財季算錯——BS 那半已修，IS 那半還開著**
   ...
   **剩下兩個，都是這次修 G13 時挖出來的獨立問題**：
   ...
   - (a) **`_current_q_col()` 會挑到「比較欄」而不是當期**——這才是那 42 份
     日期對不上的**真正根因**。實例：

     | 公司 | 表單 | IS 挑到 | BS 當期 | 差距 |
     |---|---|---|---|---|
     | ISRG | 10-Q | `2017-03-31 (Q1)` | `2018-03-31` | 一整年 |
     | KR | 10-Q | `2020-02-01 (Q1)` | `2020-05-23` | 上一財年結束日 |
     | GD | 10-Q | `2016-04-03 (Q2)` | `2016-12-31` | 三季 |
     | DELL／SNOW | 10-Q | `2023-02-03 (FY)`／`2022-01-31 (FY)` | 當期 | 季報卻挑到年度欄 |

     BS 那半修好之後，**IS／CF 的值仍然掛在 IS 挑到的那一欄**。那些值本身
     沒錯（就是該期的），錯的是「這份 10-Q 我們拿到的是比較欄的數字，當期
     那一欄沒被挑到」。要修得先弄清楚 edgartools 對這些 filing 解出來的
     dataframe 到底有沒有當期欄——**沒查證之前不要動 `_current_q_col()`**，
     年表能建起來全靠它接受 `(FY)`。
