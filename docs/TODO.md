...
> **⚠ 動手驗證任何東西之前，先讀 `docs/PITFALLS.md` 最後一節
> 「驗證與量測的陷阱」。** 那裡記的是**量測方法本身騙了我們**的七個實例，
> 每一條都先產出過一個看起來很合理、實際上是錯的數字（「真缺 57%」、
> 「97.5% 是單季」、「AAPL 缺 48 季」…）。**判準錯了，樣本再大也沒用。**
...
## ★ P2. 讓 AI 扮演「什麼都不懂的使用者」實際跑一遍（2026-09-21 CTH 指定）

**為什麼要做**：現在所有的驗收都是「資料對不對」——逐格比對、跟 SEC 原始資料
驗算、全庫掃描。那些證明了**數字是對的**，但完全沒有證明**這個工具好不好用**。
一個數字全對但使用者卡在第二步的工具，實際上還是不能用。

**怎麼做**：開一個乾淨的對話，讓 AI 扮演一個**不懂程式、不懂這個專案**的使用者：

- **只能看 `README.md`**，不可以看原始碼、不可以看 `docs/` 底下其他文件
- 照著 README 走完整流程：裝環境 → 填設定 → 輸入一個代號 → 打開 Excel
- 每一步都要記錄：**看不看得懂、卡在哪、猜錯什麼、訊息看不看得懂**
- 拿到 Excel 之後要能回答：「這家公司最近一季的營收是多少」「哪些數字是缺的、
  為什麼缺」——答不出來就是輸出的可讀性有問題

**要產出什麼**：一份「卡關清單」，每條標明是**文件問題**還是**程式問題**。

⚠ **不要讓扮演的 AI 看這份 TODO 或 CHANGELOG**——它一旦知道 G13／G14 那些
內情，就不再是「什麼都不懂的使用者」了，測不出真實的第一次體驗。

⚠ **這是黑箱驗收，跟現在做的白箱驗收互補，不能互相取代。** 現在的方法是
「用大量樣本校正邏輯漏洞」（CTH 2026-09-21 的定位），那條路找得出數字錯誤，
但找不出「使用者根本不知道該怎麼開始」這種問題。
...
G14. **CF 拆不出單季時把累計值當單季寫出去（錯誤數字，不報錯）**
   （2026-09-21 查明。裸日期那半已修，這半還開著）
   - **位置**：`_build_cf_table()` 的 YTD 拆算
     `standalone[label] = row_vals  # no prior YTD — keep cumulative as best-effort`
   - **⚠ 同一個函式裡，overflow 行拆不出來是保持 `None`，模板行卻保留累計值**
     ——兩者行為相反，而且 **overflow 行才是對的**。`docs/ARCHITECTURE.md`
     原本寫「與模板行行為一致」，那句是錯的，已更正
   - **現況影響**：裸日期修好之後大部分公司已經不需要 best-effort
     （COST 17→0、AAPL/NVDA 各 1、CHTR 0），**KR 還有 17 個**
   - **✅ KR 的根因查明（2026-09-21）：是 G13(b) 的下游後果，不是獨立問題。**
     KR 財年 1 月底結束、**Q1 是 16 週**（2 月初～5 月下旬），
     `fiscal_quarter_of()` 用月份切季（`(5-2)%12//3+1 = 2`）把 Q1 算成 Q2
     → 跟真正的 Q2 撞欄 → 退化成裸日期標籤。**季表因此完全沒有 Q1**
     （季別分布 `{Q2:18, Q3:17, Q4:17}`、12 個裸日期標籤），
     Q2 拆算找不到 Q1 當基準 → 走 best-effort。
     **所以修 G13(b) 才是治本，改動 B 只是止血。**
   - **診斷**：`fetcher_gaap.reset_cf_fallbacks()` ＋ `cf_fallbacks()` 可以列出
     一趟抓取裡有哪幾期走了這條路（只記錄、不改行為）
   - **修法**：拆不出來就留空 ＋ 記進 D11-B 缺漏帳本。⚠ 代價是那些格子從
     「有數字」變成「空白」——但**那些數字本來就是錯的**，這不是損失資料，
     是停止提供錯誤資料（對應 CTH 的要求 ①數字都要對 ②有缺漏要能發現）
...
G13. **期間欄挑錯／財季算錯——BS 那半已修，IS 那半還開著**
   ...
   **已修**：BS 跟 IS 借財季標籤時沒檢查日期，造成「對的資料掛錯日期」。
   實測 42 份／26 家（不是原本以為的 SNOW 一家——其餘 25 家是**靜默掛錯**，
   沒有症狀）。五家逐格驗收：非空格 24,905 → 25,445，沒有一張表變少。

   **剩下兩個，都是這次修 G13 時挖出來的獨立問題**：

   - (a) **有些 filing 的 IS 根本沒有「當期」欄**（2026-09-20 修掉挑欄邏輯後
     剩下的部分）。⚠ **不是挑錯，是資料不在**——挑欄那半已經修好
     （`_current_q_col()` 改取期末日最新的，18 張表／10 家，見 CHANGELOG
     2026-09-20 續一）。剩下的長這樣：

     | 公司 | 表單 | IS 有的欄 | BS 當期 | CF 有當期嗎 |
     |---|---|---|---|---|
     | ISRG | 10-Q | 只有 `2017-03-31 (Q1)` | `2018-03-31` | ✅ 有 `2018-03-31 (Q1)` |
     | GD | 10-Q | 只有 `2016-04-03 (Q2)` | `2016-12-31` | ✅ 有 `2017-04-02 (Q2)` |
     | KR | 10-Q | `2020-02-01 (Q1)`／`2019-11-09 (Q4)`／`2019-08-17 (Q3)` | `2020-05-23` | ❌ 跟 IS 一樣 |
     | DELL／SNOW | 10-Q | 只有 `(FY)` 欄 | 當期 | ❌ 只有 `(FY)` |

     **ISRG／GD 的 CF 有當期而 IS 沒有**——所以「IS 缺當期時從 CF 借期間欄」
     是一條可能的路，但那只補得到 CF 也有的科目，而且要先確認不會把 CF 的
     期間語意帶進 IS。KR／DELL／SNOW 那種**兩張表都沒有**的，這一層無解，
     要嘛走 companyfacts（`fetcher_facts.py` 現成的），要嘛接受。
     **✅ 2026-09-20 查證完畢：SEC 原始 XBRL 有資料，是 edgartools 沒解出來。**
     所以修法是「補一條取數路徑」，不是「接受資料不存在」。逐案打 companyfacts
     API 查單季 Revenue fact：

     | 公司 | filing | IS 缺的當期 | companyfacts | 期間長度 |
     |---|---|---|---|---|
     | ISRG | `0001035267-18-000048` | 2018-03-31 | ✅ 847,500,000 | 89 天 |
     | DELL | `0001571996-23-000019` | 2023-05-05 | ✅ 20,922,000,000 | 90 天 |
     | SNOW | `0001640147-22-000044` | 2022-04-30 | ✅ 422,371,000 | 88 天 |
     | KR | `0001104659-26-078236` | 2026-05-23 | ✅ 46,121,000,000 | **111 天** |

     **⚠ 查證時自己踩到一個坑，記下來免得下次再踩**：第一輪把 duration 篩在
     80~100 天，**KR 的 Q1 是 16 週＝111 天，被自己的篩選條件濾掉**，於是得出
     「KR 根本沒 tag」的錯誤結論。放寬重查才看到真相。**篩單季不可以假設
     一季≈90 天**——這跟 (b) 是同一個根源。

     **⚠ GD 不是真案例**：`0000040533-17-000014` 的「BS 當期 2016-12-31」其實是
     **上年度末的比較欄**（GD 財年是曆年），不是當期。是「拿 BS 最大日期當當期」
     這個偵測法有誤，不是 IS 缺欄。統計這類問題時要排除掉。

     **本地快取實測的份數**（10-Q，IS 沒有 BS 當期那一天）：ISRG 2、GD 3、
     ...
     **✅ 第一步（偵測）2026-09-20 完成**，見 CHANGELOG。`_note_missing_current_period()`
     比對 `period_of_report` 與報表挑到的期間欄，不符就記進 D11-B 缺漏帳本。
     **只記帳不改流程**，端到端比對 ISRG 22,359 格只有 70 格變動、**全是警告文字**
     （Index 1 格 ＋ Data_Meta 的缺漏欄 69 格），財務數字一格沒動；AAPL 0 差異。

     **⏳ 第二步（補值）待做——設計已經驗證過，但還沒實作。**

     四個步驟（五家實測驗證：ISRG／GD／DELL／SNOW／KR）：

     1. **偵測**——已完成（上面那個函式就是觸發點）
     2. **挑期間**——在 XBRL contexts 裡找 `endDate == period_of_report` 且
        `type == duration` 的，**先排除 < 60 天的**，再挑**最接近 91 天**的
     3. **撈數字**——`period_key` 相符 ＋ `statement_type` 相符 ＋
        `is_dimensioned == False`
     4. **填回去**——**只填 `dimension == False` 且非 abstract 的列**

     ⚠ **步驟 2 的兩條規則是實測調出來的，不是設計出來的。** 第一版寫「挑最短
     的 duration」，GD 與 DELL 直接選到 **0 天**的單日事件 context（股票發行日
     那種），撈到 0 個 fact。而 `85~100 天` 也不能寫死——**KR 的 Q1 是 16 週
     ＝111 天**。

     ⚠ **步驟 4 是整個設計最危險的地方。** ISRG 的損益表有 81 列但只有 27 種
     concept，**18 種各重複 4 次**（維度細項）。只用 concept 當 key 的話，一個
     總額會被填進全部 4 列——「ASC 606 - Adjustments」那列去年是 760 萬（調整
     數），會被填成 6.9 億（總額）。**數字全錯而且不報錯。** 限縮到無維度列之後
     concept 才唯一（五家都驗過），而當期撈到的 fact 也全部無維度，兩邊天然對齊。

     **怎麼驗補出來的數字**：同一家公司**後續的 filing** 會把這一期列為「去年
     同期」。ISRG `0001035267-19-000038` 就有 `2018-03-31 (Q1)` 欄——拿它當答案
     卷，而且那是**另一份 filing 的獨立 XBRL**，不是自我驗證。

     **待決**：補不滿怎麼辦（GD 只對得上 12/21 列，其餘是公司那份 filing 真的
     沒報）。傾向有多少補多少、缺的照樣進缺漏帳本，並在 `Data_Meta` 標記來源。
   - (b) **52/53 週財年制的財年年份會差一整個財年**（2026-09-20 實測重新定性，
     **原本寫的 KR 症狀量不到，真正的症狀是另一個**）

     **原本的描述**（未驗證）：「`fiscal_quarter_of()` 用月份切季，KR 的 Q1 是
     16 週、5 月下旬才結束，月份法算成 Q2」。**2026-09-20 實測 15 家 × 約 250
     個財年，KR／TGT／DELL／COST／WMT／HD／LULU／ONTO 全部 OK**——那個症狀
     現在量不到（可能是 G13 主體修完後就沒了）。

     **真正的症狀**：52/53 週制的公司財年結束日會在 **12 月底與 1 月初之間跳**，
     而 `fy_end_month` 只用「月份」表達，跳到 1 月那幾年就整個財年錯位。
     實測 15 家裡有 **JNJ（3 個財年）與 CDNS（2 個財年）**中招：

     | JNJ 標籤 | 期末日 | 真正的財年 |
     |---|---|---|
     | `FY2021Q4` | 2021-01-03 | **FY2020**（10-K filed 2021-02-22 報的是 2020 財年） |
     | `FY2021Q1` | 2021-04-04 | FY2021 ✅ |
     | `FY2021Q2` | 2021-07-04 | FY2021 ✅ |
     | `FY2021Q3` | 2021-10-03 | FY2021 ✅ |

     **同一個 `FY2021` 欄裡混了兩個財年的資料**——Q4 是 2020 年的，Q1~Q3 是
     2021 年的。權威依據是年報的 `period_of_report`（`2021-01-03` 那份 10-K
     報的就是 FY2020）。

     **⚠ 要修得動核心模型，不是修一行**：`fy_end_month` 這個「用月份表達財年
     結束」的模型對 52/53 週制**表達力不夠**。正確做法是拿**年報期末日序列**
     當錨點（判斷某個期末日落在哪兩份年報之間），跟 J11 的修法一脈相承——
     J11 已經證實 `period_of_report` 對全庫 215 家 100% 準確。但
     `fiscal_quarter_of()` 有 **7 個呼叫點**，簽名要改，而且會動到**所有公司**
     的期間標籤，驗收要全庫逐格比對。**跟 G8 同級的風險，建議獨立一輪做，
     先寫設計再動手。**

     **現在的傷害範圍**：全庫 31 家 52/53 週候選裡，用「同一個 FY 標籤下四季
     順序」判出 **CDNS 與 JNJ** 兩家、5 個財年。

     **⚠ 但那個判準有盲點（2026-09-21 發現）**：它只看得懂 `FYxxxxQn` 格式的
     標籤，而**還有一型是「標籤根本算不出來」**——`fiscal_quarter_of()` 算錯季
     之後跟別的欄撞號，退化成裸日期字串（G13 修正時加的保護），於是不符合
     格式、被檢查器整個略過。**KR 就是這型**：Q1 是 16 週、5 月下旬才結束，
     月份法算成 Q2 → 撞欄 → 退化 → **季表完全沒有 Q1**（12 個裸日期標籤），
     還連帶讓 CF 的 Q2 拆不出來（見 G14）。

     抽樣 10 家（KR／COST／TGT／DELL／HD／LOW／WMT／AAPL／ADBE／MAR）只有 KR
     中招——一般 52/53 週制是 13 週 × 4 季，**KR 的 16 週 Q1 是少數**。
     但全庫還沒量過這一型，`scripts/check_fy_labels.py` 要補上「裸日期標籤」
     這個維度才看得到。
   - (c) **季表仍可能留一欄純年度標籤**（SNOW 的 `FY2022`）。現在只丟
     「整欄皆空」的，那一欄有 4 格真值（年度 CF 的期末現金、股數）所以留著，
     期末日跟 `FY2022Q4` 重複。要處理的話是「把年度 CF 的值併進 Q4」，
     不是直接丟——⚠ **不要無聲刪數字**。
...
D9. **外國私人發行人（FPI）：ARM 已經抓得到，其餘的季度數字 SEC 上沒有**
   （2026-08-20 CTH 回報 → 2026-09-19 A 路線上線 ＋ 季度來源查證定案，
   做完的部分見 `docs/CHANGELOG.md` 2026-09-19 與（續一））

   **已經可以抓的**：沒有 10-Q 的公司自動退到 6-K（季報，用 `R*.htm` 數量
   過濾）＋ 20-F（年報）。ARM 實跑走通、四季對得上年報值、db-status 正常。

   **剩下的事**：

   - (a) **ARM 要不要加回更新名單**（CTH 決定）。ARM 已經在本地資料庫裡
     （12 份），但**不在名單上**，所以過夜更新不會碰它。

   ### ⚠ ARM 以外的 FPI：季度數字 SEC 上根本沒有（2026-09-19 實測定案）

   CTH 問「有辦法讓原本抓不到的抓到嗎？我要 quarter 資料」，**三種來源全查過**
   （完整數字見 `docs/CHANGELOG.md` 2026-09-19 續一）：

   | 公司 | 6-K 有 XBRL 三表 | companyfacts 有季度 | 20-F 準則 | 季度數字在哪 |
   |---|---|---|---|---|
   | **ARM** | ✅ 9/34 | ✅ Q1/Q2/Q3 | us-gaap | **已支援** |
   | ASML | ❌ 0/24 | ❌ 全 FY | **us-gaap** | 三表是 JPG 圖片 |
   | STM | ❌ 0/24 | ❌ 全 FY | **us-gaap** | 6-K 連 EX-99 都沒有 |
   | TM／SONY／HMC | ❌ 0/24 | ❌ 全 FY | IFRS | 決算短信 HTML 表格 |
   | IFX／NTDOY | — | — | — | 查無代號（Level I），無解 |

   **決定性的是 companyfacts**：ASML 的 11,381 筆 facts 全部來自 20-F、期別
   清一色 `FY`。所以正確說法不是「解析不出來」，是 **SEC 上沒有這筆資料**。

   ⚠ **ASML 的 EX-99.3 檔名叫 `financialstatementsusgaa.htm`，內容卻是
   Workiva 產的 `<IMG>` slides，`<table>` 數量 0**——光看附件檔名會判斷錯。

   ⚠⚠ **推翻舊結論：ASML／STM 的 20-F 是 us-gaap，不是 IFRS**
   （us-gaap 1,557／1,977 筆），三表 45／74／48 與 61／82／57 列，
   **比對層一行都不用改**。以前把它們跟 IFRS 那群歸在一起是錯的。

   **三條路，CTH 2026-09-19 決定都先不做**（這些公司的季度留給 Bloomberg）：

   - (b) **「只有年報」模式**（小，約 1 小時）。現在 ASML／STM 會**整家失敗**
     ——沒有 10-Q → 退到 6-K → 0 份含財報 → 直接 raise，年報明明抓得到卻拿不到。
     做了就能拿年報，但**拿不到季度**。
   - (c) **決算短信 HTML 解析**（大，只對日本公司）。TM 的 EX-99.1 有 507K 字、
     53 張 `<table>`，`pd.read_html` 讀得出 20 張實表，技術上可行。成本在日式
     版面規則＋IFRS 列名對照，而且**各家格式不同＝一家一套**。要做之前先講清楚
     實際要覆蓋哪幾家，只為一兩家寫規則不划算。
   - (d) **ASML 的季度**：三表只有圖片，走 SEC 就只剩 OCR／vision；
     ASML 自家 IR 有 ESEF XBRL，但那已經不是 EDGAR，超出本專案定位。
   **跟上面三條路無關、單純還沒補的一個洞**：

   - (e) **ARM 的季表 Basic／Diluted EPS 兩列全空**。ARM 用
     `us-gaap_IncomeLossFromContinuingOperationsPerBasicShare`（繼續營業部門
     版本），現行模板沒對到。可以用 Override Engine 補，也可以在
     `IS_TEMPLATE` 加同義詞——後者會影響所有公司，要先確認不會撞到既有對應。

   **這一區的教訓，兩條都要留著**：

   - ⚠⚠ **不可以靠註冊地或公司名稱判斷申報身分，兩個方向都會錯。**
     NXPI（荷蘭）／LIN（英國）／ACN（愛爾蘭）／TEL（瑞士）註冊地都在國外，
     **但全部報 10-Q**；反過來 ARM 2023 才在 NASDAQ 上市、看起來像正常美股，
     **實際只有 20-F**。決定的是股東結構與管理層所在地，**只能問 SEC**
     ——**加新公司進名單前先跑 `scripts/check_filer_type.py`**。
   - ⚠ **準則跟國籍無關**：同樣是日本公司，野村用 us-gaap、本田用 IFRS。
     ADR Level I（NTDOY／IFX）走 Rule 12g3-2(b) 豁免，SEC 根本沒資料，
     症狀是「查無此代號」，跟「有資料但格式不同」要分開看。
