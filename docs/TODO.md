...
J5. ✅ **找一個晚上把全部公司的快取跑滿**（CTH 2026-09-04 交代）——**已完成：201/201 家、13,921 份、0.98 GB**，稽核與格式驗證全過（四道閘 13,921/13,921＝100%）。分兩批跑，記錄與之後的維護指令見 `docs/local-db-runs.md`。⚠ 大批量一律用 `scripts/run_localdb_batch.sh` 分段跑——一個 process 連跑 67 家會因記憶體不足被系統中止（實測）
...
J7. ✅ **本地資料庫「裡面有哪些公司」，UI 跟 AI 都查得到**（2026-09-18 完成，見 CHANGELOG）
   - **GUI**：新分頁「資料庫總覽」，Treeview 表格 + 點標題排序 + 搜尋 + 匯出 CSV。
     純瀏覽不放清除鈕（維護留 Tab3）
   - **AI／CLI**：`src/cli.py db-status [--json]`，不連網，244 家 0.53 秒
   - **2026-09-18 後續**：新增公司後會問「要現在抓嗎」、總覽可多選後「更新
     選中的」、meta 升到 schema 2 存**真正的財報期間**（`period_oldest`／
     `period_newest`，從快取 DataFrame 欄名抽，零額外成本）、`updated_at`
     終於被讀出來顯示成「上次查」（它從 J2 就在寫，但一直沒有任何人讀）
   - **欄位語意**：主欄位是**財報期間**（schema 2 之後 meta 直接存），收件日
     那組（`oldest`／`newest`）仍並存但不佔 GUI 欄位——兩者差一整個期間，
     有測試釘住不准混用
   - **唯讀**：總覽與 CLI 走 `read_meta()`，對不上顯示「需重算」不當場自癒。
     `--rebuild` 是唯一會寫檔的路徑
   - **遺留的效能問題（沒修，另記）**：`scan_filings()` 為了讀 4 個小欄位
     （form／filing_date／cached_at／版本）把每份 70KB 的 filing JSON 整個
     `json.load()` 進來 → **2.75 秒/家、244 家重算約 11 分鐘**。這是
     `rebuild_meta` 本來就有的成本、不是 J7 引入的，而且只在「明確要求重算」
     時才會踩到（正常抓取時 meta 是即時寫的）。要修的話得讓 filing JSON 的
     小欄位可以不解整份就讀到（例如寫檔時把 `dataframes` 放最後 + 串流解析，
     或另存一份輕量索引），但後者跟 `filing_cache.py`「不維護額外索引檔」的
     原則衝突，**要 CTH 決定值不值得**

J7-舊. ⚠ ~~**本地資料庫「裡面有哪些公司」目前沒有一個 UI／AI 都能查的地方**~~（2026-09-18 CTH 提出，當天完成，原始需求記錄如下）
   - **現況**：`scripts/audit_local_db.py`（不帶參數）跟 `filing_cache.list_cached_tickers()`
     都是掃 `filing_cache/` 底下的子資料夾，能回答「已快取哪些」，但兩者都是
     開發用的 script／函式，不是使用者或 AI 一眼能查的介面。GUI Tab3 的
     「本地資料快取」面板（`main.py` 一帶）顯示的是**更新名單**
     （`config["local_db_tickers"]`）＋ 各家 meta 狀態，跟「快取裡實際有哪些」
     是兩個集合，可能對不上（名單裡但沒抓過 vs. 快取裡有但已從名單移除）
   - **要做的**：
     1. GUI 開一個「查看本地資料庫」的檢視（可以是 Tab3 既有面板延伸，或獨立彈窗），
        直接列出快取裡實際存在的公司＋涵蓋期間（可以接 J6 方向②做的 Oldest Period），
        不要只顯示更新名單
     2. CLI／AI 這邊要有一個乾淨的查詢介面（例如 `cli.py list-db` 或
        `scripts/audit_local_db.py` 補一個 `--json` 純清單模式），讓 AI 助理
        不用讀原始碼、不用掃檔案系統就能問「資料庫裡有哪些公司、涵蓋到哪年」
   - **背景**：2026-09-18 對話發現這台機器上 `%APPDATA%\SEC Financial Tools\`
     整個是空的（含 config.json、filing_cache）。**已確認沒有第二台機器**，
     所以 J5 的 201 家快取是真的沒了，不是在別的地方——成因與後續處理見 J8。
     這條 TODO 跟「找回 J5 的資料」是兩件事——即使資料重抓回來了，還是要有
     查詢介面，不然每次都要重新翻 git log／CHANGELOG

J8. ✅ **本地資料庫搬家：`%APPDATA%` → 專案資料夾 `local_db/filing_cache/`**（2026-09-18 完成，見 CHANGELOG）
   - **成因**：J5 跑滿的 201 家快取（連 `config.json`）整個消失，最可能是 GUI
     Tab3「全部清除」按鈕（`filing_cache.clear_all()`）被按過——有二次確認但
     沒有備份，按下去就是真的全刪。更根本的問題是**位置語意不對**：
     `%APPDATA%` 在 Windows 慣例上代表「系統可以清掉的快取」，但這份資料其實
     是花 SEC 網路請求時間換來的永久資料庫
   - **已完成**：`filing_cache.cache_root()` 改成預設回傳
     `<專案根目錄>/local_db/filing_cache/`，`.gitignore` 排除掉不進版控，
     `SEC_LOCAL_DB_ROOT` 環境變數供測試覆寫。`config.json` 維持在 `%APPDATA%`
     不動（重建成本低，不在保護範圍內）
   - **刻意不做**：原始 SEC filing（未解析的 XBRL/HTML）不另存一份——CTH
     確認過，只存「拆分後」那層就能達成「同一家公司不重工」的目標，原始檔案
     體積是拆分後資料的數倍，沒必要多存
   - **還沒做，要 CTH 決定**：
     1. 201 家＋新增的 40 家（見 J7 背景，美股資料夾掃到的，扣掉 3 家 20-F
        外國申報人 ASML／STM／IFX）要重新抓一次，粗估 ~10 小時
     2. GUI 的「全部清除」按鈕要不要加防呆（例如清除前自動打包備份一份
        zip 到 `local_db/_backups/`，或至少要求打字確認而不是按鈕點兩下）——
        搬家降低了「被系統清掉」的風險，但沒有降低「自己手滑點清除鈕」的風險

J9. ✅ **清單的離線退路**（2026-09-18 完成，見 CHANGELOG）
   - 原始問題：完全快取的公司在 SEC 連不上時仍然叫不出來（實測確認）
   - **實測證據**：ARLO 33 份全在本地，把 `_list_filings()` 改成直接拋
     `NetworkDownError`，`fetch_gaap_statements('ARLO', ...)` 整個失敗
   - **成因**：`_list_filings()` → `company.get_filings()` 每次都連 SEC，
     沒有快取後路。貴的部分（下載＋解析 filing 內容）確實走本地，但清單
     那一次網路是硬需求
   - **為什麼要修**：專案定位是「AI 調用此工具查財務資料」（見 README
     「本地財報資料庫」章節）。SEC 抽風或離線時，明明資料都在硬碟上卻用不了，
     跟這個定位直接衝突。也跟 D11（連續大量抓取時 SEC 偶發失敗）是同一個弱點
   - **做法**：清單抓失敗時退回用本地已快取的 accession 清單
   - ⚠⚠ **清單不可以「本地優先」，只能「網路失敗才退回」**（2026-09-18 CTH
     提出「優先從本地拿、本地沒有才連網」時釐清的關鍵區別）：

     | | 財報**內容** | 財報**清單** |
     |---|---|---|
     | 資料會不會變 | **不會**，accession 一經申報就固定（修正案是新編號） | **會**，公司隨時出新財報 |
     | 正確策略 | **本地優先**（`_filing_obj()` 現況，已經是對的） | **網路優先、失敗才退回本地** |

     清單若也做成本地優先，結果是**永遠發現不了新財報**、資料庫停在最後一次
     抓的那天**而且不報錯**——`plan_ticker()` 比對「SEC 上有的 vs 本地有的」
     整套增量設計會直接失效。這是 J6 那類「不報錯、只是資料比你以為的少」的
     失效模式，對財報工具最糟
   - **內容那一層不必動**：`_filing_obj()` 已經是「記憶體 → 磁碟 → 網路，
     抓完立刻落檔」（`fetcher_gaap.py:461-485`），完全符合要的行為
   - ⚠ **不可以靜默退回**。退回代表「這批是離線資料、可能漏掉最新一季」，
     使用者必須看得到——GUI、CLI、log 都要明示，比照 D11 缺漏帳本與 J6
     「把靜默變可見」的既有原則。靜默退回會變成「以為看的是最新財報，其實
     少一季」，對財報工具來說跟 J6 是同一類最糟的失效模式
