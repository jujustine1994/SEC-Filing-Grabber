...
從 `docs/TODO.md` 搬過來（2026-09-18）——這些都**已經判決完畢、不必再決定**，
所以不屬於 TODO，但值得長期保存，免得日後有人重新踩一次。

### ⚠ 熱跑的瓶頸已經不是網路，再加一層快取不會變快

全命中的一趟 ARLO 抓取約 10~17s，其中**約九成是比對層**（`IS/BS/CF_TEMPLATE`
逐列配對、`_synthesize_q4()`、比率、segments），純本機 CPU；網路合計只有
1~2s，讀快取檔＋還原 DataFrame 約 0.95s。**`_filing_obj()` 那條線已經降到
0.1s 等級**，所以「再快取一層」是個直覺但錯誤的方向，要再快只能優化比對層。

已經做掉最便宜的一刀（2026-09-04）：`_CachedStatement.to_dataframe()` 加 memo，
`_CachedFinancials` 一併 memo statement 物件（不然每次 new 一個，上層 memo 形同
虛設）。實測 ARLO 端到端中位數 **7.07s → 6.52s**（約 9%），`payload_to_df()`
**224 次／385ms → 99 次／160ms**。每次仍回傳 `.copy()` 保持隔離——深複製比重新
解析便宜 9.8 倍，隔離幾乎是免費的。

⚠ **引用效能數字前先確認基準**：這條原本寫「省下約 0.85s（6%）」是量錯的——
解析總成本上限就只有 0.37~0.40s，省不到 0.85s，而百分比反而低估了。原記錄的
基準是「ARLO 一趟 10~17s」，重量時熱跑只有 7s，基準本身就不同。

### 已知的小取捨（都判決為可接受，不必主動修）

- `ACCESSION_RE` 用 `$` 而非 `\Z`，允許結尾一個換行。字元類只有數字與破折號，
  不可能夾帶 `/`、`\`、`..`，路徑注入防線實質上仍然成立
- `_retry_once()` 會開第二個最外層 scope，`last_cache_stats()` 因此只反映重試
  那一趟，第一趟的命中數在 log 上消失
- 空的殘留資料夾（份數與容量都是 0）不會出現在 GUI 清單，也就永遠不會被
  「全部清除」掃到。會自癒（下次抓那家時重新寫入），只是那個空殼會一直在
- ticker 換手時（同代號換成另一家公司），冒名那趟會用新 cik 覆寫同名目錄，
...
欄位擋（見下方五道閘）。這跟同一節上面的「解析快取（`_parse_cache_scope()`）」
...
是唯一的落地狀態**——查快取一律直接問這個檔案存不存在、內容能不能過五道閘，
...
`<accession>.json`，五道閘第一關就過不了，自然落回正常下載流程並寫入新
快取檔——不需要額外的「過期」判斷。
