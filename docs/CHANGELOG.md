...
- **CF 的「裸日期欄」改當累計值處理——COST 的現金流從 34/67 期回到 67/67**
  （CTH 問「是這幾間公司的特例，還是我們邏輯出錯」——是邏輯出錯的連鎖）。
  - **因果鏈**（COST 的 17 個「累計值當單季」正好對應它的 17 份裸日期 filing）：

        1. Q2 的 CF 欄是裸日期（`'2025-02-16'`，沒有 `(Qn)` 也沒有 `(YTD)`）
           → `_is_q_col()` 與 `_ytd_col()` 都認不出來 → **整份 CF 被丟掉**
        2. Q2 空白，而且「Q2 的累計值」沒被記下來
        3. Q3 是累計值，要減 Q2 累計，但 Q2 不在
        4. 走 best-effort → **累計值被當成單季寫出去**

    第 1 步是根因，「累計值當單季」只是後果。
  - **裸日期到底是什麼？拿 SEC 原始 duration 查了 11 家、65 個樣本**：
    ...
    **沒有一個是單季。** ⚠ 先前用「IS 同一天標 `(Qn)`」推測出的「97.5% 是單季」
    **是錯的**——IS 與 CF 的期間慣例根本不同（IS 同時報單季與累計，CF 通常
    只報累計）。判準只能用 SEC 的 duration。
  - **⚠ 裸日期只在 CF 特別處理，不動共用函式**：`_col_to_quarter_label()` 對
    沒有標記的欄是「原樣傳回」，那是 **BS 的 instant 欄**要的行為（時點值本來
    就不屬於任何一季）。改共用函式會把 BS 一起改壞——實作時真的踩到，
    `test_col_to_quarter_label_instant_passthrough` 擋下來了。
  - **⚠ 取最新的裸日期欄，不是第一個**：COST Q2 的兩個裸日期是當期與去年同期，
    取錯就是拿去年的數字當今年（G13(a) 的坑）。
  - **驗收**：
    - **拿 SEC 原始資料驗算救回的數字**（SEC 只報累計，單季要自己減）：

      | 期別 | SEC 累計 − Q1 | 我們算出來 |
      |---|---|---|
      | FY2025Q2 | 6,008,000,000 − 3,260,000,000 = 2,748,000,000 | 2,748,000,000 ✓ |
      | FY2024Q2 | 5,382,000,000 − 4,651,000,000 = 731,000,000 | 731,000,000 ✓ |
      | FY2023Q2 | 5,802,000,000 − 2,610,000,000 = 3,192,000,000 | 3,192,000,000 ✓ |

    - 營業現金流：**COST 34/67 → 67/67、AZO 33/67 → 65/67、PEP 34/68 → 67/68**
    - **KR 27/64 完全不變**——它不在那 11 家裡，問題是別的（還有 17 個
      best-effort，要靠下一步處理）
    - 端到端逐格比對：**AAPL／MSFT／NVDA 各 0 差異**；COST 2,607 格變動，
      其中 **1,927 格空白→有值**、只有 11 格有值→空白
    - 測試 **1612 passed**（新增 3 條，TDD 先紅後綠）

## 2026-09-21

- **修 CF 的根本邏輯錯誤：期別標籤跟資料來自不同的欄**（CTH 指出「聽起來是我們
  邏輯出錯」——確實是）。
  - **原本**：走 YTD 路徑時「標籤跟 IS 借、資料從 CF 取」，**兩個不同的欄**。
    只要兩邊挑到不同年份，就把**去年的現金流標成今年**，而且完全不報錯。
  - **全庫 6,186 份實測**：99.9% 兩邊一致，但那 4 份不一致的全是**整年錯位**，
    而且**兩個方向都發生過**——所以「優先信 IS」或「優先信 CF」都是錯的：

    | 案例 | 症狀 |
    |---|---|
    | LMT `0000936468-18-000053` | CF 欄 `['2018-06-24', '2017-06-25 (YTD)']`，**當期是裸日期**認不出來，`_ytd_col()` 只好挑去年那欄；標籤卻跟 IS 借到當期 |
    | BMY `0000014272-13-000005` | CF 的 YTD 是當期，IS 只有**去年**的單季欄 |

  - **修法：標籤一律從「資料那一欄」自己推**，兩者就永遠同期。同時拿掉
    `is_q_col is None` 的連坐條件——IS 沒有單季欄不該讓 CF 整份消失。
  - **⚠ 實測證明舊版給的是垃圾數字**：LMT FY2018Q2 舊版顯示 **2,578,000,000**，
    那是「2017 上半年累計 3,210,000,000 − FY2018Q1 632,000,000」。SEC 原始資料
    裡 LMT **2018 上半年只有 560,000,000**——舊值跟真實金額差 4.6 倍，而且沒有
    任何警告。修正後那格變空白。
  - **驗收（端到端逐格比對）**：

    | 公司 | 格數 | 差異 | 空白→有值 |
    |---|---|---|---|
    | AAPL／MSFT／NVDA | 22,407／28,274／26,581 | **0** | 0 |
    | CHTR | 31,281 | 644 | **483（救回）** |
    | LMT | 33,810 | 195 | 2（117 格是移除錯誤數字） |

    CHTR 的營業現金流從 **36/63 期回到 50/63 期**（2012~2016 連續五年只有 Q1
    有值的問題解掉大半）；ISRG 也救回 2 期。測試 **1609 passed**（新增 2 條，
    TDD 先紅後綠）。
  - **⏳ 還沒解的**：LMT 那份的當期欄是**裸日期**（`2018-06-24`，沒有 `(Q2)`
    也沒有 `(YTD)`），所以認不出來、拿不到真實的 560,000,000。裸日期欄的處理
    是下一個機會。

## 2026-09-20（續七）

- **G13(a) 第一步：當期整期不在報表裡時，要出聲**（CTH 指定「先偵測、後補值」）。
  - **修的不是數字，是沉默。** edgartools 的期間篩選門檻會把當期整個丟掉
    （根因見上一個 commit），症狀是**整季空白而且完全不報錯**——使用者不知道
    是公司沒報、SEC 沒有、還是工具壞了。這個專案最不能接受的就是這種失效。
  - **判準用 `period_of_report`**（SEC 官方認定「這份 filing 報的是哪一期」，
    不是我們猜的）。**一個判斷涵蓋所有型**：
  ...
  - **新增 `MissingCurrentPeriod`**，`FetchLedger._classify()` 直接判 `data`
    **而且不戳 SEC**。⚠ 走到那個判斷時 filing 已經下載並解析完畢，網路顯然是
    通的；誤判成 `network` 會觸發 D11-B 的整趟重試，而**重試一百次結果都一樣**
    ——那是 edgartools 的行為，不是網路。
  - **⚠ 拿不到 `period_of_report` 一律不記**：那時無從判斷，把「不知道」報成
    「有缺漏」會讓使用者對警告失去信任，而這個專案的缺漏警告是要被當真的。
  - **只記帳，不改流程**：接在 `_build_is_table()` 的 `q_col` 後面，下面的
    `continue` 與既有行為完全一樣。
  - **驗收**：
    - 實跑 ISRG 2 筆、GD 5 筆、DELL 2 筆、SNOW 1 筆；**AAPL／NVDA 0 筆
      （沒有誤報）**
    - 畫面訊息：`[ISRG] ⚠ 有 2 期沒抓到（2018-06-30、2018-03-31）。SEC 連得上，
      是這幾期的申報資料本身讀不出來，重抓結果會一樣。`
    - 端到端逐格比對：ISRG 22,359 格**只有 70 格變動，全是警告文字**
      （Index 1 ＋ Data_Meta 缺漏欄 69），**財務數字一格沒動**；AAPL 0 差異
    - 測試 `-m "not slow"` **1607 passed**（新增 7 條，TDD 先紅後綠）

## 2026-09-20（續六）

- **預覽與主路徑的財年結束月統一到同一個依據**（承 J11，CTH 指定）。
  - **J11 修好了「預覽算錯」，但兩條路仍是兩套依據**：預覽走年報清單的
    `period_of_report`、主路徑走 `_detect_fy_end_month()` 解 10-K 全文。
    結果 215/215 相同，但那是**「兩個方法碰巧都對」，不是同一個方法**——
    哪天其中一邊改了又會分岔。
  - **統一到 `_fy_end_month_from_annuals()`**，兩邊共用。理由是全庫 215 家實測
    它跟解全文 **100% 一致**，而且**不必解全文**（那是抓取流程最慢的步驟之一，
    對還沒進本地庫的新公司差別最明顯）。
  - **⚠ 解全文保留成退路**：`period_of_report` 拿不到時照舊走
    `_detect_fy_end_month()`（連同它「失敗要記進帳本讓使用者看到警告」的既有
    行為）。**行為不會比以前差**，測試釘住這條。
  - **⚠ 拿不到一律回 `None`，不可以自己填 12**：財年結束月是所有期間標籤的
    基準，把「不知道」說成「12 月」的話 AAPL 那種公司每個標籤都差一季，
    而且不報錯。要不要退到解全文由呼叫端決定。
  - **驗收**：端到端逐格比對（vs `3b7f818`）AAPL／JNJ／TGT／ARM／KR
    **合計 118,086 格、0 差異**——依據統一了但輸出完全不變。
    測試 `-m "not slow"` **1600 passed**（新增 4 條）。

## 2026-09-20（續五）

- **修 J11：預覽的財年結束月跟主路徑不同源——JNJ／ONTO 實測差了一整個財年**
  （前一個 commit 才記成「潛在風險」，實測後升級成確認的 bug）。
  - **原本**：預覽走 `Company.fiscal_year_end` 屬性（免費，已在物件上），
    主路徑走 `_detect_fy_end_month()`（解 10-K 全文）。**兩個依據就會分岔。**
  - **全庫 215 家實測**（基準＝解全文）：

    | 來源 | 準確度 | 成本 |
    |---|---|---|
    | 年報清單的 `period_of_report` | **215/215 ＝ 100.0%** | 一次 filing 清單請求 |
    | `Company.fiscal_year_end` 屬性 | 206/215 ＝ 95.8% | 免費 |

    對不上的九家：ADI／DIS／HD／JNJ／LULU／MU／ONTO／SNDK／TGT。
  - **⚠ 「來源不一致」不等於「標籤會錯」**——逐家實測預覽 vs 實抓，九家裡
    **只有 JNJ 與 ONTO 真的錯**，而且差一整個財年（JNJ 預覽 `FY2027Q2`
    vs 實抓 `FY2026Q2`；ONTO 反向）。其餘七家被 `fiscal_quarter_of()` 的
    邊界算法救回來，是巧合不是保證。
  - **修法**：改用年報清單的 `period_of_report`。⚠ 預覽的成本上限是
    **「不要解全文」，不是「不要問清單」**——原本的註解把這兩件事混為一談，
    所以才退而用屬性。年報表單跟著季報那組走（10-Q→10-K、6-K→20-F），
    只多一次請求。年報一份都沒有（剛上市）才退回屬性。
  - **驗收**：那九家 ＋ AAPL／ARM／NVDA，**預覽與實抓標籤 12/12 全部一致**。

- **P1(b)：清掉被強制中止的 process 留下的殘留 `.tmp`**
  - `atomic_write_json()` 失敗時會自己 `unlink()`，所以正常路徑不留 .tmp。
    殘留只發生在 **process 被系統砍掉**時（`run_localdb_batch.sh` 記過實測：
    一個 process 連跑 67 家會在第 16 家因記憶體不足被中止），那時 `except`
    根本跑不到。不影響正確性，只是會累積。
  - **⚠ 不可以無條件清掉所有 `.tmp`**：檔名帶 PID 就是為了讓兩個實例並行時
    不互踩，清掉別人正在寫的那份會讓它的 `os.replace()` 失敗——**那比殘留一個
    檔案嚴重得多**。所以只清「夠舊」的（預設一小時；正在寫的 tmp 不會存在
    超過幾秒）。
  - `update_local_db()` 每家開始前順手清。整家跳過的公司也清——成本只是一次
    glob，而那正是最容易累積殘留的情況（每輪都掃到、從不寫入）。
  - 測試釘住三條：舊的會清、**新的不動**、`.json` 正式檔絕對不碰。

## 2026-09-20（續四）

- **掃一輪積久的可疑 bug，抓到五個——全是同一個根源**（CTH 指定）：D9 加 FPI
  （6-K／20-F）支援時**只改了抓取主路徑，週邊全部還寫死 10-Q／10-K**。

  | # | 位置 | 症狀 |
  |---|---|---|
  | 1 | `_fetch_gaap_impl()` | FPI + 只抓季報 → 財季標籤**錯 2 季**，靜默 |
  | 2 | `local_db_row_text()` | FPI 在 GUI「資料庫總覽」**永遠顯示未到底** |
  | 3 | `preview_sheets()` | FPI 預覽的最新期別／期末日／申報日**三欄全空** |
  | 4 | `audit_local_db.py` | 對 FPI **直接 KeyError 炸掉**（不只是誤判） |
  | 5 | 3 支腳本 | `⚠`／`✅`／`❌` 在 cp950 主控台**炸掉整支** |

  - **#1 最嚴重，而且完全靜默**。只抓季報時 `filings_k` 是空的，會探一份年報
    判財年結束月——原本寫死問 `"10-K"`，FPI 交 20-F 探不到就 fallback 成 12
    （日曆年）。ARM 財年 3 月底，實測同一個期末日 `2024-09-30` 在「季+年」是
    `FY2025Q2`、在「只抓季報」變成 `FY2024Q3`。不報錯、不警告，**整份 Excel
    的期間標籤靜靜地錯掉**（跟 G13 同一類失效模式）。抽成
    `_probe_fy_end_month(company, annual_form)` 才測得動——**難測本身就是設計
    訊號**；主路徑改用 `resolved["forms"][1]`（`_resolve_listings()` 早就算好
    了，只是沒人接）。
  - **#4 比原本判斷的嚴重**：`plan_ticker()` 內部早就走 `form_set()`，回的是
    6-K／20-F 的 key，腳本卻拿 `local_db.FORMS` 去取 → `KeyError`。這支是
    「J5 每批跑完的收尾工具」，批次裡有一家 FPI 就收不了尾。
  - **#3 要守住成本**：只有「真的沒有 10-Q」才問 6-K——多問一次對 Sony
    （1,055 份）、野村（1,244 份）是上千份的差別，對正常美股是白白一次請求。
  - **順帶**：`check_filer_type.py` 的 `sys.path` 寫死絕對路徑（`C:\Users\CTH\...`），
    換機器或改資料夾名就整支掛掉，改成跟另外兩支一樣的相對路徑。
  - **驗證（每一條都實跑，不只看程式碼）**：
    - #1 ARM 兩種抓法 6 個共同期末日**標籤 0 不一致**；回歸 AAPL／NVDA 也 0 不一致
    - #2 ARM 從「未到底」變「已到底」，AAPL／NVDA 不受影響
    - #3 ARM 拿到 `FY2027Q1`／`2026-06-30`／`2026-07-29`，跟主路徑一致
    - #4 ARM + AAPL 都跑完，ARM 正確判為「完整」
    - #5 **對照實驗**：沒 `reconfigure` 就 `UnicodeEncodeError`，有就正常印出
    - **端到端逐格比對**（vs `d37ef77`）：ARLO 12,627 格、AAPL 22,407 格、
      ARM 5,004 格，**全部 0 差異**——修正只在「只抓季報」那條路生效
    - 測試 `-m "not slow"` **1587 passed**（新增 7 條，TDD 先紅後綠）
  - ⚠ 其中一條測試（preview 不多問 6-K）是**迴歸防護**，不驅動實作，寫的時候
    就是綠的——它防的是修 #3 時讓 214 家都多問一次 6-K。誠實標註，不假裝它紅過。
