...
- **修 J10：`verify_local_db.py` 對外國私人發行人永遠誤判，而且「驗出問題」時
  會整支炸掉**（前一個 commit 剛記進 TODO，CTH 指定順手做完）。
  - **⚠ 這是既有問題，不是 fetched_keys 或 D9 造成的**——拿 `efd3182` 當時的
    腳本跑同一家，結果一模一樣。
  - **(a) FPI 誤判**：meta 一致性那段寫死 `local_db.FORMS`（10-Q／10-K），
    但 ARM 這型交的是 6-K／20-F，所以永遠印
    `meta 10-Q count=None 但實際 0 份`。**資料其實好好的**——同一次跑五道閘
    12/12 全過，只是拿錯表單去比。改用 `local_db.form_set()`，跟正式程式碼
    同一個判準，符合這支腳本「直接用正式程式碼的函式判、不另外寫一套」的原則。
  - **(b) cp950 炸掉**：`✗` 編不進 Windows 主控台預設的 cp950，丟
    `UnicodeEncodeError` 讓整支腳本掛掉。**只有在真的驗出問題時才會炸**
    ——正常路徑印不到 `✗`，所以一路沒被發現，偏偏「驗出問題」正是最需要看到
    輸出的時候。照 `cli.py` 的 `_force_utf8_io()` 模式補上 stdout/stderr
    reconfigure（`errors="replace"` 保險，編不出去的字印成 `?`，不讓一個符號
    炸掉整趟輸出）。專案裡已有 9 支腳本這樣做，這支是漏網的。
  - **驗收**（都不設 `PYTHONIOENCODING`，也就是修之前會炸的跑法）：
    - ARM：**有問題的公司 0 家**、五道閘 12/12，中文不亂碼
    - 回歸 AAPL／ARLO／NVDA（一般申報人）：**0 家有問題**，沒被改壞
    - `✗` 路徑：把一份複本的 `_meta.json` 改成 `file_count=99999`，
      印出 `[1/1] ARLO ✗ meta file_count=99999 但實際 33 份`，**跑完沒炸**

## 2026-09-20（續二）

- **修 G13(a)：`_current_q_col()` 挑「第一欄」而不是「當期」，拿去年的數字
  當今年**。這是前一則那 42 份「IS/BS 日期對不上」的**真正根因之一**。
  - **根因**：`_current_q_col()` 回傳第一個符合的期間欄，等於完全相信
    edgartools 的欄順序。多數情況第一欄就是當期，但**不保證**——CSCO
    `0000858877-16-000117`（10-K）的 IS 欄是
    `['2015-07-25 (FY)', '2016-07-30 (FY)', '2014-07-26 (FY)']`，**當期排在中間**。
  - **修法**：在期間欄裡取**期末日最新的**。⚠ YTD 欄照舊排除（累計值，
    拿來當單季會把 Q2 灌成半年——BMY `0000014272-13-000005` 的最新欄正好是
    YTD，測試釘住）；期末日解不出來的欄不參與比大小，只當退路。
  - **影響面（掃 26,270 張快取表）**：**18 張、10 家**（AZO／CSCO／FORM／GS／
    KR／LITE／ODFL／ONTO／TXN／UPS）。18 張**全部是同一種**：當期排在後面、
    第一欄是去年同期，差**一整年**。
  - **驗收（13 家端到端逐格比對，含 3 家對照組）**：

    | | 非空格 | 改值 |
    |---|---|---|
    | 10 家受影響 | 合計 **+787** | GS 121 格、TXN 130 格、ODFL 135 格… |
    | **AAPL／NVDA／JPM（對照組）** | **0 變化** | **0 格** |

  - ⚠ **一個要知道的取捨**：修好之後，**原本靠「比較欄」意外補到的最舊一期
    會消失**。KR 年表的 `FY2009`（Rev 76,148M）就是那份 2010 年 10-K 的比較欄
    填出來的——現在那份 10-K 正確地填進 `FY2010`（Rev 76,733M），FY2009 因為
    快取裡沒有更早的年報而整欄消失。LITE 的 `FY2015Q1` 同理。
    **這是對的**：一欄的數字本來就該來自它自己的申報，而不是別人的比較欄。
    真的要補那些洞是 **G8「用比較欄當 fallback 補洞」**該做的事——有規則、
    標得出來源，不是靠 bug 意外補到。
  - 測試 `-m "not slow"` **1575 passed**（新增 4 條，2 紅 2 護欄）。
  - **G13 剩下的**：`_current_q_col()` 挑對了，但有些 filing 的 IS **根本沒有
    當期欄**（ISRG 2018Q1 的 IS 只有 2017 那欄，CF 卻有當期）——那是 edgartools
    上游的解析結果，這一層挑不出來。細節記在 TODO G13 (a)。


## 2026-09-20（續一）

- **加 `fetched_keys`：分得清「這張表不存在」與「當初根本沒抓」**
  （CTH 指定；起因是問「以後更新分類或修 bug 會不會要重抓」）。
  - **問題**：`dataframes` 裡的 `null` 有兩個意思，光看值分不出來——這家公司
    真的沒有這張表（正常），或者存檔當下的 `STATEMENT_KEYS` 還沒有這張表、
    根本沒去抓。代價 2026-09-18 付過一次：三張擴六張時舊檔的
    `statement_of_equity` 被讀成「這家公司沒有股東權益變動表」，**不報錯、
    Excel 默默少一張表**，只能升 `SCHEMA_VERSION` 整庫作廢重抓 11 小時。
  - **修法**：`save_filing()` 把當下的 `STATEMENT_KEYS` 明寫進 `fetched_keys`；
    `load_filing(require_keys=...)` 成為**第五道閘**，要的表不在清單裡就回
    `None`（視同無快取、照舊重抓），跟既有四道閘同一個路數，呼叫端不必多學
    一種「半有效」狀態。預設 `require_keys=CORE_STATEMENT_KEYS`——日常產
    Excel 不該因為某張沒人在讀的 extras 沒抓就整份重抓。
  - **⚠ 負向快取跳過這道閘**：`has_financials=False`（pre-XBRL）根本沒有
    `dataframes`，「哪張表沒抓」對它沒有意義。拿 `require_keys` 去擋它會讓
    那些舊申報每趟都重打 SEC、永遠不收斂。
  - **舊檔走推導，不改檔、不升版**：既有 14,417 份都沒有這個欄位，但它們全是
    `schema_version=2`，而 v2 的定義就是「六張表全抓」，所以 `load_filing()`
    讀到缺欄位時直接推導補上（結果寫回 `entry`，下游一律讀
    `entry["fetched_keys"]`，不必每個呼叫點各自判舊檔）。
    **`SCHEMA_VERSION` 維持 2 沒動。**
  - **驗收**：對全庫 14,417 份實跑 `load_filing()`——**全部讀得出來、0 份被擋、
    全部推導成六張**，檔案一個位元組都沒改。測試 `-m "not slow"`
    **1580 passed**（新增 5 條，TDD 先紅後綠）。
  - **端到端逐格比對**（開 `efd3182` 的 worktree 產同一家公司的 Excel 再比）：

    | 公司 | 路徑 | 比對格數 | 差異 |
    |---|---|---|---|
    | ARLO | 一般美國申報人 | 12,627 | **0** |
    | ARM | 外國私人發行人（6-K/20-F，跟 `filing_cache` 有耦合） | 5,004（非空 3,293） | **0** |

    ⚠ **光比輸出不夠**——萬一全部走網路重抓，快取層根本沒被執行到，0 差異就
    沒有意義。所以另外量了命中率：ARLO 33/33、ARM 12/12、AAPL 75/75、
    NVDA 75/75，**195 份 100% 命中**，確認改動的那層每一份都執行到了。
  - **立下的規矩**（寫進 ARCHITECTURE 與 `filing_cache.py` 開頭）：**加欄位前
    先問能不能從舊檔既有資訊推導出來，能就不要動 `SCHEMA_VERSION`。**
    升版是核彈級動作，全庫作廢重抓 11 小時。
  - **沒做的事**：不跑 backfill 改寫那 14,417 份。理由是 backfill **省不掉**
    程式裡那段推導判斷（只要有任何一份漏掉——腳本中斷、備份還原、從別台電腦
    複製——程式還是得能處理沒有 `fetched_keys` 的檔案），所以重寫 1.6 GB
    換來的只有「檔案總管打開肉眼看得到那一行」，不划算。

## 2026-09-20

- **修 G13：BS 跟 IS 借財季標籤時沒檢查日期，導致「對的資料掛錯日期」**
  （成因 2026-09-04 查明、修法一直沒做；CTH 2026-09-19 指定先修這條，理由是
  「其他項目最壞是空白，G13 是重複／錯位的季——空白看得見，錯位看不見」）。
  - **根因**：BS 的欄是裸日期（instant），推不出財季，所以標籤一律跟同一份
    filing 的 IS 借（`fetcher_gaap.py:1670`）。⚠ **但沒檢查兩邊是不是同一天。**
  - **修法**：日期對得上才借，對不上就用 BS 自己的期末日推算
    （`_bs_quarter_label()`）。⚠ 年報要推成年度標籤——推成 `FY2017Q4` 的話
    年表混進季標籤，`_synthesize_q4()` 會拿它當季度值去減（實作時真的踩到，
    GD／KR 的年表長出 `FY2017Q4`／`FY2010Q4`，被測試擋下來）。
  - **⚠⚠ 影響面比 TODO 記的大得多：實測 42 份、26 家**（掃 13,155 份有三表的
    本地快取）。TODO 原本寫「只有 SNOW 一家」——那是只統計**看得見的症狀**
    （期末日重複）的結果，其餘 25 家是**靜默掛錯**，沒有任何症狀。
  - **修好的樣子**（五家逐格比對，`old` vs `new`）：

    | 公司 | 修好了什麼 |
    |---|---|
    | SNOW | Goodwill 502,614,000 從 `FY2022`（期末 2022-01-31）搬回 `FY2023Q1`（2022-04-30），整季回來 |
    | ISRG | `Short-term Investments` FY2017Q1 1,542.6M → 756.2M，同時 FY2018Q1 空→1,542.6M（整整差一年） |
    | GD | `Total Assets` FY2016 **Q1/Q2/Q3 原本三季都是同一個值** 33,172M → 31,702／31,769／32,668 |
    | KR | 9 個裸日期欄（`2010-05-22` 這種垃圾標籤）變回正常財季 |

  - **⚠ 過程中自己製造又修掉一個資料損失（最重要的一段）**：推算的季**會算錯**
    ——`fiscal_quarter_of()` 用月份切季，KR 的 Q1 是 16 週、5 月下旬才結束，
    月份法算成 Q2，撞上真正的 Q2 之後 `if label in periods: continue` 把那份
    **整份跳過**，實測 KR 9 個期別的資產負債表（每份 35~38 格）直接消失。
    修成「撞欄就退回借來的標籤、再退回裸日期」——標籤不好看但**資料留著**。
    這條的教訓不是「要算對季」，是**「算錯的時候不可以順手把資料弄不見」**。
  - **BS 現在會記自己的 `period_ends`**（欄名就是期末日，零成本）。沒記的話
    只存在於 BS 的那一季（修好後回來的 SNOW FY2023Q1）期末日是空字串，
    而 `period_ends` 是 G6 補欄、日曆季對齊、Excel 期間標籤共同的依據。
  - **季表不再留「純年度標籤而且整欄皆空」的空殼欄**（`_merge_financials()`
    用 `sheet_name` 的 `(Q)` 把關——季／年表天然分開，不必像 TODO 原本評估的
    那樣穿過 8 個呼叫點）。⚠ **只丟空的**：SNOW 的 `FY2022` 欄還留著，因為它
    有 4 格真值（年度 CF 的期末現金、股數）——寧可讓人看到一欄怪標籤，
    也不要無聲刪數字。
  - **驗收（五家端到端逐格比對）**：季表＋年表非空格 **24,905 → 25,445
    （+540）**，**沒有任何一家、任何一張表變少**；年表沒有混進季標籤。
    測試 `-m "not slow"` **1571 passed**（新增 8 條，TDD 先紅後綠）。
  - **順帶挖出兩個獨立問題，已記進 TODO**：`_current_q_col()` 會挑到比較欄
    （那 42 份日期不符的**真正根因**）、`fiscal_quarter_of()` 對 52/53 週
    財年制算錯季。


## 2026-09-19（續一）

- **量出「ARM 以外的 FPI 到底能不能拿到季度數字」——答案是 SEC 上根本沒有**
  （CTH 問「有辦法讓原本抓不到的那些抓到嗎？我要 quarter 資料」，TODO D9）。
  六家實測，**三種來源全部查過**，不是憑印象：

  | 公司 | 6-K 有 XBRL 三表 | companyfacts 有季度 | 20-F 準則 | 季度數字實際在哪 |
  |---|---|---|---|---|
  | **ARM** | ✅ 9/34 份 | ✅ Q1 400／Q2 621／Q3 625 筆 | us-gaap | **已支援** |
  | ASML | ❌ 0/24 | ❌ 全 FY（10,529 筆全來自 20-F） | **us-gaap** | 三表是 **JPG 圖片** |
  | STM | ❌ 0/24 | ❌ 全 FY | **us-gaap** | 6-K 連 EX-99 都沒有 |
  | TM 豐田 | ❌ 0/24 | ❌ 全 FY | IFRS | 決算短信 HTML，53 張表／20 張 ≥8 列 |
  | SONY | ❌ 0/24 | ❌ 全 FY（6-K 有 834 筆但期別仍是 FY） | IFRS | 同 TM 型態 |
  | HMC／NMR | ❌ 0/24 | — | IFRS／us-gaap | 同上 |

  - **決定性的一條是 companyfacts**：ASML 11,381 筆 facts **全部來自 20-F、
    期別清一色 `FY`**。對照 ARM 的 2,756 筆來自 6-K、有 Q1/Q2/Q3。
    所以「解析不出來」是錯的說法——**是 SEC 上沒有這筆資料**。
  - ⚠ **ASML 的三表是圖片**：EX-99.3 檔名寫著 `financialstatementsusgaa.htm`
    （看起來大有希望），實際內容是 Workiva 產的 `<IMG>` slides，
    **`<table>` 數量 0**。EX-99.1 新聞稿只有營運指標表（Total net sales、
    機台出貨數），沒有完整三表。**光看附件檔名會做出錯誤判斷**。
  - **TM 型態反而有料**：EX-99.1（決算短信）507K 字、53 張 `<table>`，
    `pd.read_html` 讀得出 20 張實表。技術上可行，成本在日式版面規則＋IFRS
    列名對照，而且**各家格式不同＝一家一套**。
  - ⚠⚠ **推翻一條舊結論：ASML 與 STM 的 20-F 是 us-gaap，不是 IFRS**
    （ASML us-gaap 1,557 筆、STM 1,977 筆）。三表解出來 45／74／48 列與
    61／82／57 列，**比對層一行都不用改**。D9 原本把它們跟 IFRS 那群歸在
    一起，是錯的。
  - **順帶發現一個現存缺口（還沒修，CTH 決定先不做）**：這兩家的年報明明
    抓得到，但現在會**整家失敗**——沒有 10-Q → 退到 6-K → 過濾後 0 份含財報
    → 直接 raise。要拿年報得先有「只有年報」模式。
  - **CTH 決定：三條路（只有年報模式／決算短信解析／維持現狀）都先不做**，
    這些公司的季度數字留給 Bloomberg。研究結論寫進 TODO D9 備查。


## 2026-09-19

- **D9 A 路線上線：外國私人發行人的 6-K／20-F 抓得到了，ARM 實跑走通**
  （CTH 指定範圍：只做 us-gaap 的 FPI，IFRS 先不做）。
  - **抓取端**：沒有 10-Q 的公司退到 **6-K（季報）＋ 20-F（年報）**
    （`_resolve_listings()`）。⚠ **只有「沒有 10-Q」才去問 6-K**——正常美股
    214 家連問都不問，那對 Sony 那種公司是上千份的差別。
  - **6-K 過濾**：用 `R*.htm` 檔數量 > 1 判斷哪幾份含財報，判定結果存進
    **獨立檔 `_sixk_probe.json`**（不併進 `_meta.json`，判定不是財報內容）。
    ARM 實測：34 份 6-K → 留 9 份，第一輪 17.6 秒（34 次 index 請求）、
    **第二輪 0.0 秒**。
  - **狀態層**：`local_db` 的 `FORMS` 從寫死的 `("10-Q","10-K")` 改成
    **每家公司自己的表單組**（`form_set()`）。寫死的話 FPI 會整組判空：
    「到底了沒」永遠算不出來、meta 永遠判不可沿用，**兩種症狀都不報錯**。
  - **ARM 實跑對帳（數字自己對得起來）**：季報 12 季、年報 3 年。
    FY2025 四季營收 939＋844＋983＋1241 ＝ **4007 ＝ 年報值**；
    FY2026 是 1053＋1135＋1242＋1490 ＝ **4920 ＝ 年報值**；
    營運現金流 FY2025 −290＋6＋423＋258 ＝ **397 ＝ 年報值**
    ——**證明 6-K 的現金流量表跟 10-Q 一樣是 YTD 累計、切單季的邏輯直接通用**，
    Q4 從 20-F 反推（`_synthetic_q4`）也照舊成立。`db-status` 顯示
    12 份／期間 2023-09-30~2026-06-30／到底了 yes／meta_ok true，
    第二輪整家跳過。跨公司比較 `compare ARM NVDA` 也吃得到。
  - ⚠⚠ **實測抓到一個會讓 A 路線失效的 bug，而且是靠 TM 才發現的**：
    原本 `_r_file_count()` 讀不到 index 回 `0`，跟「真的 0 個 R 檔」撞在一起。
    **ARM 沒財報的 6-K 是 1 個 R 檔，TM（豐田）的是 0 個**——TM 的 634 份
    6-K 因此全部被當成「判定失敗，保守留著」。改成讀不到回 `None` 之後
    重測：TM 前 40 份 → **留 0 份**。這條推翻 TODO 原本寫的「沒財報的
    全部都是 1 個」，那是只看 ARM 一家得到的結論。
  - ⚠ **已知缺口（照 D9 原本量的一致）**：ARM 的季表 **Basic／Diluted EPS
    兩列全空**。ARM 用的是 `us-gaap_IncomeLossFromContinuingOperationsPer*Share`
    （繼續營業部門版本），現行 `IS_TEMPLATE` 沒對到。年報 FY2024 那欄有值。
    對照組 ARLO 是 19/24、ARM 17/24，差的就是這兩列。
  - **`scripts/check_filer_type.py` 改成四分類**：正常／FPI 但抓得到／
    只抓得到年報／查無代號（ADR Level I）。加新公司進名單前照舊先跑。
  - 測試：新增 `tests/test_foreign_issuer.py`（14 條）＋ `test_filing_cache.py`
    5 條 ＋ `test_local_db.py` 9 條，全部 TDD 先紅後綠。
    **做過反向驗證**：拿掉 `save_sixk_probe()` 那行 → 「index 只問一次」
    那條確實變紅。`-m "not slow"` **1562 passed**。


## 2026-09-18（續十六）

- **修 `overnight_update_db.ps1` 的 log 鎖檔錯誤**（實跑第 15 段開始反覆出現）。
  畫面不斷噴 `Add-Content : 由於另一個處理序正在使用檔案 '...overnight_*.log'`。
  - **根因**：`Add-Content` 每次呼叫都「開檔→寫→關檔」，密集寫入時會跟自己
    前一次的關檔還沒完成撞上（防毒即時掃描會放大這個窗口）。
  - **抓取本身完全不受影響**——報錯的下一行就是 `-> updated（新增 75 份）`，
    只有那幾行 log 掉了。但畫面被錯誤訊息洗版，**看起來像出大事**，真正重要
    的訊息被埋掉，這才是要修的理由。
  - **修法**：改用 `[System.IO.File]::AppendAllText()` ＋ 最多重試 5 次
    （20/40/60/80/100ms 退避），五次都失敗就放棄那一行——**寫 log 失敗絕對
    不能中斷抓取**。壓測 600 行密集寫入：零錯誤、中文正常。
  - **當晚那一輪照舊跑完沒有停**：實測 16/27 段時 **更新 128 家、失敗 0 家、
    D11 缺漏 0 家**，資料是乾淨的，沒有理由重跑。
  - 順手釐清畫面上其他看起來嚇人但正常的訊息：`SGML fetch failed... falling
    back to homepage`（edgartools 自己退到備用路徑並成功）、
    `[GD] 診斷：['Diluted EPS'] 在 EDGAR 中找不到對應概念`（Override Engine
    的提示，**不是抓取失敗**，TODO D11 早就記過這條容易被誤讀）。
...
- **量出現行比對層對三種來源的命中率，確認「us-gaap 的 6-K 幾乎免費」**
  （TODO D9）。CTH 問「比較一下和現行機制解析的差異」，拿損益表模板
  （24 列）實跑 `_match_is_row()`：
  - **ARLO 10-Q（對照組）19/24**、**ARM 6-K（us-gaap）17/24**、
    **TM 20-F（IFRS）10/24**。
  - ⚠ **對照組自己也只有 19/24**——缺的 5 列（D&A CF memo、Total Costs、
    Minority Interest、NI incl. NCI、SBC）是正常的，不是每家都在損益表表面
    列這些。**19/24 就是「好」的狀態**，別誤以為 24/24 才算成功。
  - **us-gaap 的 6-K 只差 2 列**（Basic/Diluted EPS），現行模板直接可用。
    剩下的工作是「怎麼把 6-K 納入清單」，不是「怎麼解析」。
  - **IFRS 缺的是 Gross Profit／R&D／Interest Expense 等核心科目**，證實工程量
    在科目對照表而非解析器。

## 2026-09-18（續十三）

- **六家外國發行人實測，CTH 的 ADR 分級框架完全成立**
  （`scripts/probe_foreign_issuer.py`，TODO D9）。CTH 提供 ADR Level I/II/III
  與 SEC 申報要求的對照後補測 TM／SONY／HMC／NMR／NTDOY／ARM。
  - **Level I 走 Rule 12g3-2(b) 豁免＝SEC 根本沒資料**。NTDOY（任天堂）查無
    此代號，跟 IFX 同一種症狀——「查無代號」是 Level I 的指紋，跟「有資料但
    格式不同」要分開看。
  - ⚠ **又推翻 D9 一條**：「要另建一套 IFRS 解析邏輯」是錯的。edgartools
    **已經**解析得了 IFRS 的 20-F——TM 56／146／52 列、SONY 124／160／77 列、
    HMC 94／182／61 列，三表全部出得來。真正的工程量在**下游**：
    `IS/BS/CF_TEMPLATE` 用 us-gaap 科目名比對，IFRS 的列名對不上。
    **要建的是科目對照表，不是解析器**，成本差很多。
  - **準則跟國籍無關**：同樣是日本公司，**野村用 us-gaap、本田用 IFRS**。
    跟「註冊地猜不準」是同一個教訓，只能實測。
  - **ARM 的 6-K 季報是特例**：日本四家各掃 12 份 6-K，**0 份有財報**。
    所以「6-K 裡有季報」目前只在 ARM 身上看到。
  - ⚠ **這推翻了原本傾向的「B 路線」**（把 6-K 無條件加進清單、靠負向快取
    過濾）：日本公司的 6-K 沒財報，而 **Sony 有 1,055 份、野村 1,244 份**
    ——無條件加進去等於每家多下載上千份無用檔案。**判準必須是動態的**
    （列清單時用 R 檔數量過濾＝A 路線），多那次 index 請求比起省下上千份
    下載完全划算。
  - 工具留在 `scripts/probe_foreign_issuer.py`，之後想評估任何一家外國公司
    直接跑，不必再憑印象猜。

## 2026-09-18（續十二）

- **實測推翻 D9 的兩條結論：ARM 的季報其實解析得出來**。CTH 追問「ARM 應該有
  SEC 資料才對」「6-K 沒有 XBRL 嗎」，追下去發現前人的研究（2026-08-21，
  拿 TSM／UMC／NBIS 測）**不能套到所有外國私人發行人**。
  - **ARM 在 SEC 有 157 份申報**，財報是完整的：3 份 20-F（年報）＋ 33 份 6-K。
    先前說「抓不到財報」講得太含糊——不是沒資料，是**這個工具只讀 10-K/10-Q**。
  - **推翻一：20-F 不一定是 IFRS**。ARM 的 20-F 有 **1,248 筆 `us-gaap`**
    標籤（＋arm 自訂 180），不是 IFRS。所以科目對照層一行都不用動。
  - **推翻二：6-K 有 XBRL**。33 份全部都有，其中 **9 份含完整財報**
    （568~876 筆 us-gaap）。原本的說法「6-K 沒有 XBRL」不精確——正確說法是
    「大部分 6-K 的 XBRL 只有 8 筆 dei 封面標籤」。⚠ 我第一次抽查剛好抽到
    沒財報的那份就下結論，掃完 33 份才看到真相。
  - **實測可解析**：2026-07-29 那份 6-K 走既有的 `filing.obj().financials`
    → 損益表 51 列、資產負債表 65 列、現金流量表 47 列，**結構跟 10-Q 一樣**。
  - **季度覆蓋完整**：ARM 財年 3 月底結束，Q1→8 月、Q2→11 月、Q3→2 月各一份
    6-K，Q4 併在 20-F。三份季報 ＋ 一份年報 ＝ 四季齊全。
  - **找到 33/33 完全分離的自動判準**：`R*.htm` 檔數量 > 1。有財報的 61~72 個，
    沒財報的**全部都是 1 個**。R 檔是 SEC 財報檢視器從 XBRL 三表產生的。
    對照組 `EX-101.CAL` 只有 32/33（2024-08-12 是假陽性）。
  - ⚠ **範圍**：全部是 ARM 一家的觀察。TSM／UMC 前人實測是 IFRS，對它們仍然
    不通。正確結論是「**用 us-gaap 的外國發行人很接近可支援，用 IFRS 的才是
    真的工程量大**」，不是「外國發行人都抓不到」。
  - 兩條可行路線（預先用 R 檔數過濾 vs 讓既有的負向快取機制處理）都寫進
    D9，還沒做，要 CTH 決定。

## 2026-09-18（續十一）

- **實測掃出更新名單裡 4 家抓不到的公司，並做成可重複的工具**
  （`scripts/check_filer_type.py`，TODO D9）。
  CTH 問「名單裡會不會還有日本公司之類的也會出問題」。與其憑印象猜，
  直接問 SEC 每家有幾份 10-Q——這個工具靠 10-Q 抓季報，沒有 10-Q 就是抓不到。
  - **218 家全掃，實際 4 家有問題**（不是先前說的 3 家）：ARM／ASML／STM
    只有 20-F，IFX 在 SEC **查無此代號**（Infineon 在美國是 OTC ADR `IFNNY`，
    `IFX` 是德國交易所代號）。四家已移出名單，218 → 214。
  - **日本公司：這批裡沒有。** 掃描用實際行為判定而非代號猜測，有的話會被撈出來。
  - ⚠⚠ **最重要的教訓：不可以靠註冊地或公司名稱判斷，兩個方向都會錯。**
    - NXPI（荷蘭）／LIN（英國）／ACN（愛爾蘭）／TEL（瑞士）註冊地都在國外，
      **但全部報 10-Q**——SEC 認定它們是美國國內申報人
    - 反過來 **ARM 是先前憑印象列清單時漏掉的**：2023 才在 NASDAQ 上市、
      看起來像正常美股，實際只有 20-F
    - 決定申報身分的是股東結構與管理層所在地，不是註冊地
  - 工具留在 `scripts/check_filer_type.py`，**加新公司進名單前先跑**，
    比事後在 10 小時的抓取裡看它失敗便宜得多。

## 2026-09-18（續十）

- **TODO 大掃除：610 → 559 行，把「已完成」與「不是待辦」的內容請出去**。
  CTH 問「TODO 是不是有點太長、有沒有做完卻沒搬走的」。逐條查證（沒查證就刪
  會弄丟紀錄）：
  - **H1 刪除**——companyfacts 對 CF 流量項的覆蓋率 25% → 100%，已完成且
    CHANGELOG 2026-09-04 有完整記載，直接刪（20 行）。
  - **I7／I6 搬到 `ARCHITECTURE.md`**（新增「快取層：已判決的取捨與效能
    天花板」一節，36 行）。這兩條**都不是待辦事項**：I7 是「熱跑九成成本在
    比對層，再加一層快取不會變快」的效能天花板與已做掉的 memo 優化，I6 是
    四條已判決為可接受的小取捨。放 TODO 會讓人以為還要做，但它們值得長期
    保存——所以是搬不是刪。CHANGELOG 原本沒有 memo 那條，先搬再刪才不會弄丟。
  - **過期敘述修正**：I 區塊還寫著「尚未併回 master」、J 區塊寫著「J1-J4
    未 push、未併 master」——兩個都已經在 master 上了。
  - **D9 接上今天的實例**：CTH 的更新名單裡真的有三家 20-F 外國私人發行人
    （ASML／STM／IFX），今晚會失敗在這三家，那是預期行為。
  - **保留的 ✅ 是進度標記不是完成條目**：H4「第一步已完成、第二步待做」、
    J6「方向②已做、①③不做」、P1(a) 這種，父項目還開著，刪掉會讓人不知道
    做到哪裡。TODO 的維護規則講的是「完成的**條目**」，不是「開放條目裡的
    進度」。
  - 剩下 559 行多半是 G／H 兩區的實測證據（G13 的重複列成因、H4 的延伸 tag
    量化），那些是還沒做完的事，證據留著才判斷得了要不要做。

## 2026-09-18（續九）

- **TODO 整理＋中斷續跑的自動化防線（P1，CTH 指定最優先）**。
  - TODO 照它自己的維護規則清了一次：J1-J5、J7-J9 做完的條目**直接刪掉**
    （內容都在 CHANGELOG），先前標 ✅ 留著是違反規則的。J6 因為 CTH 決定
    不做①③，保留條目並寫明理由。
  - 新增 **★ 最優先 P1**：本地資料庫抓滿一次 11 小時，「半夜斷掉不能白費」
    是這套東西能用的前提。實測三種中斷點都能自動接續——公司之間
    0.7 秒/家跳過、某家抓一半只補缺的（移走 5 份 → 命中 40、下載 5）、
    檔案寫一半的半截檔不被採信。
  - **把三條保證寫成測試**（原本只有手動實測，沒有防線）：逐份即時落檔、
    `atomic_write_json()` 的 tmp + `os.replace()`、`plan_ticker()` 用
    「SEC 清單 vs 本地 accession」比對。任何一條被破壞，中斷續跑都會
    **靜默失效**，而且要等到下次真的斷電才發現。
  - ⚠ **四條測試都做了反向驗證**（全綠不等於有在檢查）：把 `plan_ticker()`
    改成一律跳過 → 續跑那 2 條確實變紅；`.tmp` 那條比較有意思——拆掉
    `glob("*.json")` 或 `ACCESSION_RE` **任一道都還是綠的**，兩道都拆才變紅，
    代表那個行為本來就有雙重防線，而測試釘的是行為不是某個實作。
  - 剩下的 P1(b) `.tmp` 累積清理與 (c) 重跑固定成本 3-4 分鐘，都只是體感問題，
    不影響正確性。
  - 測試：`-m "not slow"` **1534 passed**。
...
- **TODO J1-J4 完成：本地財報資料庫的狀態層**（分支 `feat/local-filing-db`，
  ...
  - **端對端驗證：深度快取真的變成可用的資料**（AEP，2026-09-04）。
    AEP 原本快取裡只有 21 份（約 5 年），拓到底之後 75 份。拿它跑一次
    `cli.py gaap AEP --max-filings 80`：
    ...
    | `Data_Ratios` | 69 期 | FY2009Q2 → FY2026Q2 |

    **耗時 17 秒**，filing 一份都沒下載（只有列清單那一次網路）。
    這條驗的是 J5 真正的價值：拓到底不只是「磁碟上多了檔案」，
    而是把每家公司的可用歷史從 5 年變成 17 年，而且之後每次重產只要十幾秒
  - **15 家規模實測（2026-09-04，這是 J5 的前置量測）**：拿快取裡 15 家只有
    5 年份的公司拓到底。

    | 輪 | 耗時 | 結果 |
    |---|---|---|
    | 第一輪 | 32m09s | 15 家全部更新，**新增 783 份**，0 失敗、**0 缺漏** |
    | 第二輪 | 29s | 14 家跳過；ACN 補了 2 份（第一輪沒抓齊） |
    | 第三輪 | **12s** | **15/15 全部跳過** |

    - **D11 的缺漏率是 0**（`gap_tickers` 空的，783 份一份沒漏）。連續抓半小時
      沒有觸發 SEC 的偶發失敗——**所以 J5 可以直接跑，不必先做 D11 (c) 降速**
    - 冷跑速率 **2.46 s/份**（783 份／1,930s）
    - ⚠ **ACN 那 2 份是有意義的觀察**：第一輪沒抓齊，第二輪才補上，第三輪才跳過。
      也就是「跑一輪不保證到底」，**跑完隔天再跑一輪很便宜（12 秒）而且會自動補**。
      這正是增量設計要的行為，但也代表 J5 跑完之後**要再跑一輪確認全部跳過**，
      不能看到第一輪跑完就當結束
  - **J5 的推估（用上面的實測值換算）**：201 家 × 每家約 78 份 ≈ 15,700 份，
    扣掉現有的 1,689 份 → 約 14,000 份新的 × 2.46s ≈ **9.6 小時**、
    磁碟約 **1.36 GB**（跟設計書估的 1.4 GB 對得上）
...
- **TODO G13 成因查明：不是「重複日期」，是「對的資料掛錯日期」**。原本猜「同一期
  被兩份 filing 用不同財季標籤收進來」，**猜錯了**；查下去發現症狀比原本記的嚴重。
  - **單一一份 filing 造成**：`0001640147-22-000044`（2022-06-03 申報，本該是
    FY2023Q1、期末 2022-04-30）。edgartools 解出來 **IS/CF 的期間欄只有
    `2022-01-31 (FY)`、根本沒有 Q 欄**；BS 則是裸日期 `['2022-04-30', '2022-01-31']`
  - **鏈路**：`_is_q_col()`（`fetcher_gaap.py:875`）把 `"FY"` 也算成期間欄 →
    `_current_q_col()` 回傳它 → `_col_to_quarter_label()` 回 `FY2022`；
    而 **BS 的標籤是跟 IS 借的**（`fetcher_gaap.py:1487`），值取自 `2022-04-30`、
    標籤取自 `FY2022`
  - **⚠ 結果是資料正確性問題**：那個 `FY2022` 欄有 36 格，全是**2022-04-30 的
    資產負債表**掛在 `2022-01-31` 底下。跟真正的 `FY2022Q4` 共有值的 31 列裡
    **28 列數字不同**（Goodwill 502,614,000 vs 8,449,000——SNOW 2022 年 3 月併購
    Streamlit，5 億是併購後才有的）。**使用者看到兩欄同一個日期、資產負債表互相
    矛盾，而且正確的是比較舊的那一欄**；FY2023Q1 這個標籤則整個消失
  - **頻率**：掃 201 家答案卷，「季表出現純年度標籤」與「期末日重複」都只有 SNOW
    一家且一對一重合——同一個成因的兩個症狀
  - **修法未做，且原本寫的 (a) 案「季表直接拒收 `(FY)` 欄」實測後不建議單獨做**：
    9 份 10-K（SNOW/AAPL/JPM）的 IS 期間欄**全部只有 `(FY)`**，年表能建起來完全
    靠 `_is_q_col()` 接受 `"FY"`——直接拒收會讓年表全空、`_synthesize_q4()` 算不出
    Q4，**每家公司每年的 Q4 全部變空**。要做成「只有季表拒收」得穿過 6 個 builder、
    8 個 `_current_q_col()` 呼叫點與 11 個 builder 呼叫點，而且拒收後 BS 標籤會
    退化成裸日期字串。四層風險與新浮出的 (d) 方向都寫在 TODO G13
...
- **快取替身的 DataFrame memo（TODO I7 第一刀）**：四個 builder（IS／BS／CF／
  ...
  結果，`_CachedFinancials` 一併 memo statement 物件（不然每次 new 一個，
  上層 memo 形同虛設）。
  - **實測（ARLO，預設 80/20，各跑 5 次，範圍完全不重疊）**：
    端到端中位數 **7.07s → 6.52s**（省 0.55s，**約 9%**）；
    `payload_to_df()` **224 次／385ms → 99 次／160ms**
  - **⚠ 修正 TODO I7 原本記的「省下約 0.85s（6%）」**：那個秒數是高估的
    （解析總成本上限就只有 0.37~0.40s），百分比反而低估。原記錄的基準是
    「ARLO 一趟 10~17s」，這次量到的熱跑是 7s，基準本身就不同
  - **每次仍回傳 `.copy()`，不共用同一個 DataFrame 物件**。全庫查過沒有任何
    一處改動報表 dataframe（零 `inplace=True`、零欄位指派），共用「現在」是
    安全的；但哪天有人寫 `df["x"] = ...`，症狀會是「另一張表莫名多一欄」，
    極難查。實測深複製比重新解析便宜 **9.8 倍**（0.17ms vs 1.67ms），
    隔離幾乎免費
  - **驗收**：5 家（ARLO／AAPL／GOOGL／META／JPM，含金融股）memo 前後逐格
    比對 **23,859 格 0 格不同**；非 slow 測試 1392 → **1396 passed**（+4）
  - 端到端省下的 0.55s **大於**解析本身省下的 0.22s。多出來那段**沒有查證
    原因**（合理猜測是少配置 125 個 DataFrame 的記憶體壓力，但沒量過）


- **四顆會發動抓取的按鈕互鎖（TODO I3）**：原本 Tab1／批次／掃描靠
  `is_running` 互鎖，跨公司比較卻只鎖自己那顆按鈕、不看別人——「批次抓到
  一半切到第 4 分頁按產生比較 Excel」是按得下去的，兩趟會同時對 SEC 發請求。
  D11 已實測連續大量抓取會讓 SEC 偶發失敗、**靜默少掉幾格**，所以這是資料
  完整度問題不是效能問題。
  - 新增純函式 `main.any_fetch_running(is_running, compare_running)` 與
    `_sync_run_buttons()`，四顆按鈕（Tab1 執行／批次執行／快速掃描／產生比較
    Excel）由同一處統一換算狀態；`_start_worker()` 與比較的發動點都改成看
    兩個旗標防重入
  - **刻意不讓比較去設 `is_running`**：那個旗標同時是 `_start_worker` 的
    防重入鎖，而比較有自己的 `compare_done`／`compare_error` 還原路徑，
    共用會讓還原點交錯。兩個旗標各自維護、統一換算
  - 順帶補上 `_scan_running`：快速掃描原本只 disable 自己那顆按鈕、沒有旗標，
    統一同步會在掃描途中把它誤放開並蓋掉「掃描中」字樣
  - Tk 探針實測 10/10（含「比較跑到一半 `_start_worker` 不會再開一趟」與
    「掃描途中不會被誤放開」兩條）

- **`tests/test_i18n.py` 新增第四道防線：同一個語言檔不得有重複 key（TODO I4）**。
  2026-09-03 真的發生過——加快取字串時 `en.py`／`ja.py`／`zh_cn.py` 各被多塞
  一份 `gui.btn.confirm`，三條既有測試**全綠**，因為 Python 的 dict 字面值在
  解析階段就把重複鍵吃掉了，執行期的 `STRINGS` 永遠只有一份。當時是靠
  `git diff --stat` 行數對不上才發現，那不是可以依賴的防線。新測試改用 `ast`
  讀原始碼，並附一條反向測試確認偵測器本身真的會抓（全綠不等於有在檢查）。
...
- [x] **本地 filing 快取（2026-09-03，分支 `feature/local-filing-cache`，Task 1-11）**：
      ...
      - **實測（ARLO，GAAP，10-Q+10-K，33 份 filing，2026-09-03，重量版——
        ...
        7.89s（33/33 命中）。⚠ **不要引用單一倍數**——2026-09-03 同機多輪重量，
        冷跑 55~110s、熱跑 8~17s，倍數落在 3.5~9.5 倍之間，冷跑取決於 SEC
        當下反應速度、熱跑取決於本機 CPU 負載，只有同一次連續執行內的比值
        有意義。穩定可引用的是 `cache 0/33 → 33/33` 這個精確計數，以及
        `scripts/excel_golden.py check` **0 格不同**。另外實測發現熱跑剩下的
        時間**九成是比對層的本機運算、不是網路**（詳見 ARCHITECTURE），
        所以再加快取不會讓熱跑更快。冷跑本身因為 miss 時要
...
- [x] **跨公司比較視窗的顯示修正 ＋ log 顯示耗時、全面改英文（2026-09-02）**：
      **① 已選公司的 chip 只顯示 ticker**（CTH 截圖回報）
      - 原本是 `NVDA NVIDIA CORP`，三家就把兩行塞爆。公司全名在選的當下已經
        在建議清單看過，chip 只是「已選了誰」的指示
      - 抽 `company_chip_entries()` 成模組層級純函式，測試釘得住

      **② chip 換行位置會歪掉（真的是 bug）**
      - 根因：`pack_wrapped_chips()` 把 chip 建成**外層容器**的子元件，再用
        `in_=rows[-1]` 借行 frame 排版。父子關係與排版容器不一致，第二行以後
        的 chip 位置就跑掉（CTH 截圖：第三個 chip 跑到視窗右邊）
      - 改成 chip 直接建在該行底下：先建在目前行、量寬度，放不下就砍掉、開新行
        重建。chip 數量是個位數到二十幾，重建成本可以忽略
      - **指標 chip 用同一支函式**，那邊名稱長、更常換行，一起修好
      ...
      **④ log 顯示「這次跑了多久」，三條線都補齊**
      - 新增 `format_elapsed()`：`48s` / `1m 54s` / `2h 02m 05s`。原本
        `{秒//60}分{秒%60}秒` 在三個地方各寫一次，抽出來統一，不足一分鐘不再
        寫成 `0m 48s`
      - **跨公司比較整條線原本完全沒有落檔 log**——沒有任務起始行、沒有結果行、
        當然也沒有耗時，出事只能靠使用者口述。補上起始標頭（公司清單／頻率／
        期間／指標數塞在同一行，符合「出錯時要知道當時是什麼設定」）與三個失敗
        出口 + 成功出口的結果行
      - 單一抓取／批量抓取的結果行改成**畫面與 log 檔同時有**：畫面走 `t()` 跟著
        介面語言，log 檔那份固定英文

      **⑤ log 檔訊息一律改英文**（CTH 決定，取代原本的「固定繁中」）
      - 三個理由：① 這個檔 `launcher.ps1`（PowerShell）與 `main.py` 兩邊都在寫，
        而 Windows 主控台是 cp950，中文＋符號在這條路徑上炸過（PITFALLS 地雷五／
        地雷十有記）——全英文整類避開；② 不跟著介面語言漂，切日文介面時格式不變、
        `grep` 得到同一組關鍵字；③ 這份 log 的讀者是維護者與 AI，不是使用者
      - **一次換完不留混雜**：Python 端 12 行 + PowerShell 端 11 行全部改掉。
        **畫面上給使用者看的中文一行都沒動**
      - ⚠ 這與全域規則 `~/.claude/project-rules/windows-tool.md` 的「log 檔固定用
        母語言」牴觸。**全域規則檔沒有動**（那是 CTH 的檔），例外記在這裡與
        `docs/ARCHITECTURE.md`；要推廣到其他專案由 CTH 自己決定

      **驗收（不是只看程式碼）**：實際把「選擇比較內容」視窗建起來檢查 widget 狀態
      ——chip 文字 `['NVDA', 'MSFT', 'INTC']`、建議清單 `winfo_manager()` 為空（已收起）、
      3 個 chip 的父元件都是 `TFrame`（它們那一行）；另外把 `LOG_FILE` 導到暫存檔跑
      真正的比較 worker 失敗路徑，確認落檔長這樣：
      ```
...
- [x] **G6：抓不到的季度不再整欄消失，留一整欄空白（2026-08-25）**：
      - **CTH 原話**：「要做，我寧願他是空白欄」。現況欄位清單是「成功抓到什麼就放
        ...
        看不出中間漏了一季
      - **兩條線都改**：單一公司 `fetcher_gaap._merge_financials()`（新增
        `_with_gap_columns()` 與 `_next_fiscal_label()`）、跨公司
        `comparison_writer.visible_layout()`（新增 `_fill_period_gaps()`）
      - **判定公式共用 `data_quality.missing_quarters()`**（`round(天數差 / 91) - 1`，
        單一缺口上限 4 季），沒有另外寫第二份。那條公式與上限是 52 家、1,482 對相鄰
        期間實測定下來的：111~150 天那 16 筆**全部是 COSTCO 的 16 週第四季**
        （112~119 天），`round(112/91) = 1` 正確判為沒缺——**固定門檻（例如
        「>120 天算缺」）會把它們全部誤判成缺一季**。兩邊都有回歸測試釘住這個案例
      - **跨公司這條要逐公司從期末日算，不能看「日曆季標籤有沒有連號」**：COSTCO 的
        16 週第四季在對齊季上本來就會跳過一格（沒有任何一季的中點落在那個日曆季），
        照標籤連號補會補出一堆假缺口
      - 只補在最早與最新之間（CTH 已定），不往更早補；跨公司只補「沒有任何一家抓到」
        的那一季——別家有抓到的話那一欄本來就在
      - 補出來那一欄沒有真實期末日：單一公司第 5 列退回由財季標籤反推的年月
        （`2025-06`，`_fiscal_period_end()`），跨公司的期末結算日列整格留空。
        **不編一個不存在的完整日期**——`fiscal_input._apply_to_sheet()` 看到非 ISO
        日期就保留靜態標籤（G1 已把那條路徑的殘留錯值收乾淨），Snapshot 與圖表也不會
        把假日期當成真的期間
      - **會讓更多 TTM 比率變 `None`，這是正確的**（本來就缺，以前看不出來），
        記在這裡免得日後被當成回歸
      - **`max_filings` 的語意要重讀一次**：它限制的是抓幾份 filing，不是最後有幾欄。
        補完缺口欄之後欄數可以比 `max_filings` 多（`test_fetch_passes_max_filings`
        已改成數「真的抓到的期間」）
      - `override_engine.check_key_rows()` 只看最後 4 欄，插入的空白欄會讓那個視窗
        往前挪。缺口一定在中間（最新那一欄永遠是真資料），所以不可能因此變成「最近
        4 期全空」，但一列若原本只有較舊那兩欄有值，視窗挪動後會被判成缺——**評分
        標準沒有動**（那是 G5 的事），實測影響見下方 TODO G5
      - 15 個新測試（`test_fetcher_gaap.py` 7、`test_comparison_writer.py` 8、`test_excel_writer.py` 1
        的端對端：空白欄第 5 列是 `2025-06` 這種非 ISO 日期，走完整條寫檔路徑不會炸，
        第 1 列保留靜態標籤、資料格是空的）
      - **驗收（NVDA 實跑，2026-08-25）**：53 欄裡補出 1 欄空白（FY2014Q4）；
        **`override_engine.check_key_rows()` 在補欄前後完全相同**（IS/BS/CF 三張表都是空清單）
        ——TODO 特別要求先確認「會不會誤傷 Index 完成度」，實測不會。原因是缺口一定在中間
        （最新那一欄永遠是真資料），湊不出「最近 4 期全空」

- [x] **G2 + G7：跨公司輸出加「日曆季 ↔ 財季」對應表與「說明」sheet（2026-08-25）**：
      - **G2 對應表**：`Compare_Data` 最上方、第一個指標區塊之前，一格講完
        「這一欄對這家公司是哪一財季、期末日幾號」（`FY2026Q2 (0727)`）。下面的
        指標區塊只給日曆季，不重複財季
      - **為什麼用對應表而不是「只寫每家公司的財年開始月份」**：對應表每一格都是逐期
        從實際期末日算出來的，公司哪一年改過財年，那一欄自己就會反映出來，**不需要
        任何例外處理**；財年開始月份只寫得下一個值，公司改過財年就直接失效。有測試
        釘住「中途改財年」的情境
      - **前置改動**：`ComparisonResult` 多帶 `fiscal_labels`（`{ticker: {日曆季:
        財季}}`）與 `synthetic_q4`（`{ticker: {推算 Q4 的日曆季}}`）。後者不另外從
        `fetcher_gaap` 拉旗標一路傳下來——季報表裡出現 Q4 一定是推算的（SEC 沒有
        Q4 的 10-Q，那一欄只可能來自 `_synthesize_q4()`），由財季標籤直接判斷，
        少掉一條會在二十幾處 `StatementTable(...)` 重建中被靜默弄丟的傳遞鏈
      - **最容易壞的地方**：插入對應表把所有列號往下推。`write_snapshot_sheets()`
        與 `write_chart_sheets()` 都吃 `block_ranges`，測試改成**回頭查那個列號在
        Compare_Data 上是誰**，不比對列號常數
      - **順手收攏**：「哪些公司、哪些欄位會出現在表上」抽成 `visible_layout()`，
        對應表、各指標區塊、說明 sheet 三邊共用一份。各指標區塊的欄位因此一律等寬
        （原本各自篩選，而 `write_chart_sheets()` 用的是全表 `max_column`，窄的區塊
        會讀到別人的欄位）
      - **G7 說明 sheet**（英文 sheet 名 `Notes`，擺在 `Compare_Data` 之後）：12 條
        定義說明，每條帶「這份檔案是否真的踩到」的勾選與「實際情況」欄（如
        「NVDA 1 月、AVGO 11 月、AMD 12 月」「最大差 29 天（2025Q2）」）。判定一律
        由實際資料算出來，結構性的條目永遠勾
      - **資料驅動**（CTH 明講這張表未來會擴充）：`NOTE_ITEMS` 一條一行
        `(標題鍵, 內文鍵, 判定函式)`，新增一條只要加一行 + 四個 locale 各加兩條，
        文字完全不寫死在版面程式裡。測試釘住「四個語系都查得到每一條的 key」
      - **第 10 條「本檔案缺少的公司」是這張表最重要的一條**：抓取失敗原本只寫進
        GUI log，檔案裡完全看不出來——`INTC_NVDA_AMD_TSM_v3.xlsx` 檔名有 TSM、
        使用者也選了 TSM，但檔案裡只有三家（TSM 報 20-F 不是 10-K），沒有錯誤訊息、
        沒有空欄位、圖上就是三條線。這條只在真的有公司失敗時才佔一列
      - **原草稿有一條寫錯，已刪掉**：第 5 條後半句「`Other (as reported)` 區不補」
        對這份檔案是錯的——跨公司可選指標只來自三張模板 + `RATIO_DEFS`，overflow 列
        根本不在選單裡
      - 四個語系各補 40 條字串（39 條說明 sheet + 1 條對應表標題）
      - 20 個新測試（`test_comparison.py` 4、`test_comparison_writer.py` 16）
      - **驗收（六家真實資料實跑，2026-08-25）**：AMD／NVDA／AVGO／INTC／MRVL／LITE
        （財年結束月 12／1／11／12／2／6 月），`output/_compare/G2G7G6_accept.xlsx`
        - 對應表逐格核對正確：NVDA `2025Q2 → FY2026Q2 (0727)`、AVGO `2024Q3 → FY2024Q4 (1103)`、
          MRVL `2026Q1 → FY2027Q1 (0502)`、LITE `2025Q1 → FY2025Q3 (0329)`
        - 說明 sheet 自己算出來的財年結束月「NVDA 1 月、MRVL 2 月、LITE 6 月、AVGO 11 月、
          AMD 12 月、INTC 12 月」**六家全對**
        - 六家 0 失敗，所以第 10 條（本檔案缺少的公司）正確地整列不出現
- [x] **列 filing 清單也補上退避重試（2026-08-25）**：`company.get_filings()`——比
      `_filing_obj()` 抓內容更早一步——完全沒被任何重試機制蓋到。2026-08-25 用
      D11-B 剛做完的機制驗收 201 家重建時，撞到 6 家逾時（JCI/JPM/MCD/MDLZ/MDT/META），
      全部發生在這一步：一次瞬斷直接讓整趟 `fetch_gaap_statements()` 拋例外，連
      D11-B 的缺漏帳本都還沒開始記，重試不到。新增 `_list_filings()`，套用跟
      `_filing_obj()` 同一套 `with_retry`（2/4/8 秒退避），四個呼叫點都接上。
      2 個新測試、全套非連線測試 1134 passed，JCI 實跑確認正常
- [x] **D11-B：偵測到帳本有網路造成的缺漏，自動重試一次並合併結果（2026-08-24）**：
      - **背景**：D11 實測發現連續大量抓取時 SEC 偶發失敗會靜默少掉資料（少了年報就
        `_synthesize_q4()` 合成不出 Q4）。D11 的另一個修法 (b)——Index 上顯示這次抓取
        失敗幾份——查證發現**早在 2026-08-17 就做過了**（`Data_Meta` 的 `Fetch Gaps`／
        Index A3 橘底列），這次只做 (a)：自動重試
      - **順帶查證並修正一筆誤記**：D11 原本列的「11 家出現『自動修復：發現 N 期缺失
        需要修復重疊』」候選名單經查證是誤記——全庫搜尋「重疊」兩字，程式碼裡完全沒
        有這句話。實測那批公司 log 印的其實是 Override Engine 的「在 EDGAR 中無對應
        概念」警告，跟 filing 抓取失敗無關，是完全不同的既有資料缺口類別。已在
        `docs/TODO.md` D11 註記作廢
      - **設計**：`fetch_gaap_statements()` 跑完第一輪後，若 `led.network_blamed`
        （有任何一期是網路造成的缺漏），退避 5 秒後用同一組參數整趟重跑一次
        （`_retry_once`，開自己獨立的 `collect_gaps()` 範圍），兩份結果逐格合併
        （`_merge_retry_tables`：原本有值的格子保留，`None` 的格子用第二輪的值補）。
        帳本用 `FetchLedger.absorbed_by_retry()` 吸收——同一個 `where` 標籤在重試那輪
        沒有再失敗就判定救回來了，從帳本移除；`Data_Meta` 的 `Fetch Gaps` 欄位也跟著
        更新（`_patch_meta_gap_note`），不然 Index 會顯示重試前的舊缺漏數
      - **只在 `network_blamed` 時觸發**：`kind=="data"` 代表 SEC 有回應、是資料本身
        的性質，重試沒用，維持 `fetch_ledger.py` 既有的分類判斷，不重新發明
      - **只重試一次**：不遞迴重試到底，避免網路真的斷線時無限拖時間；重試路徑本身
        意外出錯（非預期例外）會 fail-safe 退回第一輪結果，不會把「已經抓到大半資料」
        變成整趟失敗
      - **為什麼整趟重跑而不是只補失敗的那幾份**：帳本現在只記期間標籤跟例外分類，
        沒存 filing 物件本身；要做到「只重試失敗的那幾份」得動 7 個 `_note_gap()`
        呼叫點的呼叫鏈，範圍與風險都大得多。單一 ticker 重跑一次多花的時間可接受，
        因為只有真的撞到 `network_blamed` 才會觸發
      - **接線位置踩過一個坑**：一開始把重試邏輯放在 `fetch_gaap_statements()` 的
        `_ledger() is None` 遞迴入口，結果 `main.py`／`cli.py` 都是自己先開
        `collect_gaps()` 才呼叫進來，永遠不會走到那個分支——等於重試邏輯是死碼。
        改成接在兩條路徑都會經過的下游（讀當下 `_ledger()`，不管是誰開的）才修正
      - 12 個新測試（`test_fetch_ledger.py` 3 個、`test_fetcher_gaap.py` 9 個），
        全部純函式、不碰網路；全套非連線測試 1132 passed；另用 KO 實跑一次確認
        正常路徑（無缺漏）不受影響、不會誤觸發重試
