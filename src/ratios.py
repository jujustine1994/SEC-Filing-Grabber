...
RATIO_CATEGORIES: list[str] = [
    "Growth", "Profitability", "Structure", "Cash Flow", "Efficiency", "Leverage", "Returns", "Per Share",
]
...
RATIO_DEFS: list[RatioDef] = [
    # ── 成長 ────────────────────────────────────────────────────────────
    ("Revenue YoY (%)", "Revenue[t] / Revenue[t-4] - 1", "Growth",
     lambda c, i: _growth(c, "Revenue", i, 4)),
    ("Revenue QoQ (%)", "Revenue[t] / Revenue[t-1] - 1", "Growth",
     lambda c, i: _growth(c, "Revenue", i, 1)),
    ("Gross Profit YoY (%)", "Gross Profit[t] / Gross Profit[t-4] - 1", "Growth",
     lambda c, i: _growth(c, "Gross Profit", i, 4)),
    ("Operating Income YoY (%)", "Operating Income[t] / Operating Income[t-4] - 1", "Growth",
     lambda c, i: _growth(c, "Operating Income", i, 4)),
    ("Net Income YoY (%)", "Net Income[t] / Net Income[t-4] - 1", "Growth",
     lambda c, i: _growth(c, "Net Income", i, 4)),
    ("Net Income QoQ (%)", "Net Income[t] / Net Income[t-1] - 1", "Growth",
     lambda c, i: _growth(c, "Net Income", i, 1)),
    ("EPS YoY (%)", "Diluted EPS[t] / Diluted EPS[t-4] - 1", "Growth",
     lambda c, i: _growth(c, "Diluted EPS", i, 4)),
    ("Shares Outstanding YoY (%)", "Shares Outstanding[t] / Shares Outstanding[t-4] - 1", "Growth",
     lambda c, i: _growth(c, "Shares Outstanding", i, 4)),
    ("Gross Profit QoQ (%)", "Gross Profit[t] / Gross Profit[t-1] - 1", "Growth",
     lambda c, i: _growth(c, "Gross Profit", i, 1)),
    ("Operating Income QoQ (%)", "Operating Income[t] / Operating Income[t-1] - 1", "Growth",
     lambda c, i: _growth(c, "Operating Income", i, 1)),
    ("EPS QoQ (%)", "Diluted EPS[t] / Diluted EPS[t-1] - 1", "Growth",
     lambda c, i: _growth(c, "Diluted EPS", i, 1)),
    ("FCF YoY (%)", "Free Cash Flow[t] / Free Cash Flow[t-4] - 1", "Growth",
     lambda c, i: _growth(c, "Free Cash Flow", i, 4)),
    ("EBITDA YoY (%)", "EBITDA[t] / EBITDA[t-4] - 1", "Growth",
    ...
    ("Opex Ratio (%)", "Total Operating Expense / Revenue", "Profitability",
     lambda c, i: _pct(_safe_div(_at(c, "Total Operating Expense", i), _at(c, "Revenue", i)))),
    ("R&D Ratio (%)", "R&D Expense / Revenue", "Profitability",
     lambda c, i: _pct(_safe_div(_at(c, "R&D Expense", i), _at(c, "Revenue", i)))),
    ("SG&A Ratio (%)", "SG&A Expense / Revenue", "Profitability",
    ...
    ("EBITDA Margin (%)", "(Operating Income + D&A) / Revenue", "Profitability",
     lambda c, i: _pct(_safe_div(
         ...
         _at(c, "Revenue", i)))),
    ("Pre-tax Margin (%)", "Pre-tax Income / Revenue", "Profitability",
    ...
    # ── 結構 ────────────────────────────────────────────────────────────
    ("D&A / (COGS + Opex) (%)", "D&A / (Cost of Revenue + Total Operating Expense)", "Structure",
     lambda c, i: _pct(_safe_div(
         _at(c, "D&A", i),
    ...
    ("Non-op / Pre-tax (%)", "Total Non-op Income/(Loss) / Pre-tax Income", "Structure",
     lambda c, i: _pct(_safe_div(_at(c, "Total Non-op Income/(Loss)", i),
                                 _at(c, "Pre-tax Income", i)))),
    ("Effective Tax Rate (%)", "Income Tax / Pre-tax Income", "Structure",
     lambda c, i: _pct(_safe_div(_at(c, "Income Tax", i), _at(c, "Pre-tax Income", i)))),
    ("SBC / Revenue (%)", "SBC / Revenue", "Structure",
     lambda c, i: _pct(_safe_div(_at(c, "SBC", i), _at(c, "Revenue", i)))),
    ("SBC / Operating Cash Flow (%)", "SBC / Operating Cash Flow", "Structure",
     lambda c, i: _pct(_safe_div(_at(c, "SBC", i), _at(c, "Operating Cash Flow", i)))),
    ("D&A / Revenue (%)", "D&A / Revenue", "Structure",
     lambda c, i: _pct(_safe_div(_at(c, "D&A", i), _at(c, "Revenue", i)))),
    ("EBITDA ($mm)", "Operating Income + D&A", "Structure",
     lambda c, i: _ebitda(c, i)),
    ("Total Debt ($mm)", "Short-term Debt + Current Portion of LT Debt + Long-term Debt", "Structure",
     lambda c, i: _total_debt(c, i)),
    ("Net Debt ($mm)", "Total Debt - Cash", "Structure",
     lambda c, i: _net_debt(c, i)),
    ("Working Capital ($mm)", "Total Current Assets - Total Current Liabilities", "Structure",
    ...
    ("Quick Ratio (x)", "(Total Current Assets - Inventories) / Total Current Liabilities", "Leverage",
     lambda c, i: _safe_div(
         ...
         _at(c, "Total Current Liabilities", i))),
    ("Net Debt / EBITDA (x)", "(Debt - Cash) / TTM EBITDA", "Leverage",
     lambda c, i: _net_debt_to_ebitda(c, i)),
    ("Interest Coverage (x)", "Operating Income / abs(Interest Expense)", "Leverage",
     lambda c, i: _safe_div(
    ...
    ("Debt-to-Equity (x)", "Total Liabilities / Total Equity — Parent", "Leverage",
     lambda c, i: _safe_div(_at(c, "Total Liabilities", i), _at(c, "Total Equity — Parent", i))),
    ("Equity Multiplier (x)", "Total Assets / Total Equity — Parent", "Leverage",
     lambda c, i: _safe_div(_at(c, "Total Assets", i), _at(c, "Total Equity — Parent", i))),
    ("LT Debt to Capital (%)", "Long-term Debt / (Long-term Debt + Total Equity — Parent)", "Leverage",
     lambda c, i: _pct(_safe_div(
    ...
    # ── 報酬率（TTM 淨利 ÷ 期初期末平均）──────────────────────────────────
    ("ROE (%)", "TTM Net Income / avg Total Equity — Parent", "Returns",
    ...
    ("ROA (%)", "TTM Net Income / avg Total Assets", "Returns",
     lambda c, i: _pct(_safe_div(_ttm(c.get("Net Income"), i, _labels(c)),
                                 _avg(_at_lag(c, "Total Assets", i, 3),
                                      _at(c, "Total Assets", i))))),
    ("ROIC (%)", "Operating Income x (1 - Effective Tax Rate) / (Total Debt + Equity - Cash) [approx.]", "Returns",
     lambda c, i: _roic(c, i)),

    # ── 每股 ────────────────────────────────────────────────────────────
    ("BVPS ($)", "Total Equity — Parent / Shares Outstanding", "Per Share",
     lambda c, i: _safe_div(_at(c, "Total Equity — Parent", i),
                            _at(c, "Shares Outstanding", i))),
    ("FCF per Share ($)", "TTM Free Cash Flow / Shares Outstanding", "Per Share",
