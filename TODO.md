...
>
> - [ ] 欄寬（A 30 / B 34 / C 30 / 數據欄 13）夠不夠、中文說明欄會不會被切
> - [ ] 凍結窗格在 D3，但表頭有 5 列——往下捲看不到第 3~5 列，要不要改 D6（見 D0-4）
> - [ ] 三表底色（IS 深藍 / BS 藍綠 / CF 紫 / overflow 灰）與 5 列間隔看起來對不對
> - [ ] 數字格式：金額 ÷1M 千分位、百分比 `0.0%`、每股兩位小數、`(x)` `(days)` 後綴
> - [ ] **新字型**：文字微軟正黑體、數字 Consolas 等寬，位數有沒有真的對齊
> - [ ] **Index 新字級**：A1 16 / A2 10 / 表格 11 / B4 輸入格 12 / 提醒 10
> - [ ] **Index 提醒那列（A5）列高 32**，wrap 的長字串尾巴會不會被切掉
> - [ ] `Index!B4` 改一個數字，`Data_Financials(Q)/(Y)` 第 1、3、4 列有沒有跟著變
>
