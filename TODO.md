...
## D0b. 排版調整（2026-08-08 CTH 指定，未做）

D0b-1. **`Index` 字體大小要調**：目前 A1 抬頭 14pt、A2 資訊列 9pt、表格 10pt、財年提醒 9pt。CTH 覺得要改，**確切要調成多大要問他**——這是視覺判斷，不要自己猜一個數字改下去。

D0b-2. **整份文件內文字體設為「微軟正黑體」**：現在沒有指定字型，Excel 會用預設（多半是 Calibri／新細明體），中文說明欄看起來很雜。
   - 影響範圍：`excel_formatter.py` 裡所有 `Font(...)` 呼叫都沒帶 `name=`，要統一補上
   - 注意兩件事：① openpyxl 的 `Font` 是不可變的，改字型要整個重建，不能只設 `.name`；② 標題／section header 那些已經指定顏色與粗體的地方要一併帶字型，漏掉的話同一張表會混兩種字體，比全部都是預設還醜
   - 建議做法：在 `excel_formatter` 定義一個 `FONT_NAME = "微軟正黑體"` 常數，所有 `Font()` 統一吃它，日後要換字型只改一處
   - **數字欄要不要一起換要先問**：等寬數字（Calibri／Consolas）對齊比較好看，中文字型的數字通常不等寬
