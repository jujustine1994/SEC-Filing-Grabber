...
>       （`fiscal_input._wrapped_row_height()`），七份實測皆 94.5。**待 CTH 複驗**
...
>
> **七份都已用最新程式重產完畢（2026-08-08 22:17）並自動驗過**：字型只有
> 微軟正黑體 + Consolas 兩種、九個關鍵列位七家全對、A5 列高 94.5、品質 9/9
> （COHR 8/9 是舊有的 `Operating Income` 缺口，無 AI key 時 E2 診斷不啟動）。
> 數字抽查量級合理；唯一要 CTH 自己看一眼的是 **PLTR 最新季淨利率 54.9%**，
> 對其營運偏高，疑似一次性稅務項目，無外部來源可對。
>
